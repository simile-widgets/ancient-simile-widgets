...
curl -s --data "mode=scrape&url=http://www.senate.gov/general/committee_assignments/assignments.htm&time=1000&scraper=http://simile.mit.edu/~ryanlee/assignment.js" http://127.0.0.1:$PORT/ | sed -e 's/^{//' -e 's/}$//' | sort | uniq > scraped.n3

# Ask Babel for JSON
curl http://simile.mit.edu/babel/translator?reader=n3&writer=exhibit-json&url=$WHEREAMI/scraped.n3 > senate-committees.js
...
curl -s --data "mode=scrape&url=http%3A%2F%2Fthomas.loc.gov%2Fcgi-bin%2Fbdquery%2F%3F%26Db%3Dd$CONGRESS%26querybd%3D%40BAND(%40FIELD(FLD010%2B%40eq($DATE))%2B%40FIELD(FLD007(11000)))&time=1000&scraper=http://simile.mit.edu/~ryanlee/thomas-scraper.js" http://127.0.0.1:$PORT/ | sed -e 's/^{//' -e 's/}$//' | sort | uniq > scraped.n3

# Put the bills in a date-specific file
curl http://simile.mit.edu/babel/translator?reader=n3&writer=exhibit-json&url=$WHEREAMI/scraped.n3 > senate-bills.js.$DATE
