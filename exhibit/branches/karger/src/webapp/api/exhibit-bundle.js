...
/* namespace-xml-importer.js */
Exhibit.XMLImporter={};
Exhibit.importers["application/xml"]=Exhibit.XMLImporter;
Exhibit.XMLImporter.getXMLDocument=function(B){var A=null;
$.ajax({url:B,type:"GET",dataType:"xml",async:false,success:function(C){A=C;
}});
if(A){return A;
}else{alert("ERROR FINDING XML DOC");
return ;
}};
Exhibit.XMLImporter.appendUserPropertyToArray=function(C,E,A){var B=E.propertyTags.indexOf(C.nodeName);
var D=A[E.propertyNames[B]];
if(typeof A[E.propertyNames[B]]=="string"){D=[D];
D.push(C.textContent);
}else{D.push(C.textContent);
}return D;
};
Exhibit.XMLImporter.appendPropertyToArray=function(B,D,A){var C=A[B.nodeName];
if(typeof C=="string"){C=[C];
C.push(B.textContent);
}else{C.push(B.textContent);
}return C;
};
Exhibit.XMLImporter.getItems=function(E,D,C,F){var B=this;
var A=F.itemTag[C].replace(":","\\:");
$(A,E).each(function(){var L=[];
var G=[];
$(this).children().each(function(){G.push(this);
});
objectToAppend={};
while(G.length){var K=G.pop();
var H=B.determineType(K,F);
if(H=="property"){if(L.indexOf(K.nodeName)>=0){if(F.propertyTags.indexOf(K.nodeName)>=0){objectToAppend[F.propertyNames[C]]=B.appendUserPropertyToArray(K,F,objectToAppend);
}else{objectToAppend[K.nodeName]=B.appendPropertyToArray(K,F,objectToAppend);
}}else{if(F.propertyTags.indexOf(K.nodeName)>=0){var I=F.propertyTags.indexOf(K.nodeName);
objectToAppend[F.propertyNames[I]]=K.textContent;
}else{objectToAppend[K.nodeName]=K.textContent;
}}L.push(K.nodeName);
}else{if(H=="Item"){var I=F.itemTag.indexOf(K.nodeName);
var J=B.configureItem(K,{},F,I);
objectToAppend[J.type]=J.label;
}else{if(H=="fakeItem"){$(K).children().each(function(){G.push(this);
});
}else{alert("error: nodetype not understood");
}}}}objectToAppend=B.configureItem(this,objectToAppend,F,C);
D.items.push(objectToAppend);
});
return D;
};
Exhibit.XMLImporter.getParentItem=function(C,B){if(C.parentNode==null){return null;
}else{if(B.itemTag.indexOf(C.parentNode.nodeName)>=0){var A=B.itemTag.indexOf(C.parentNode.nodeName);
return this.configureItem(C.parentNode,{},B,A);
}else{this.getParentItem(C.parentNode,B);
}}};
Exhibit.XMLImporter.configureItem=function(D,C,E,B){if(!(C.label)&&E.propertyLabel[B]!=null){C["label"]=$(E.propertyLabel[B],D)[0].textContent;
}else{C["label"]=$(D).children()[0].textContent;
}if(!(C.type)&&E.itemType[B]!=null){C["type"]=E.itemType[B];
}else{C["type"]=D.nodeName;
}var A=this.getParentItem(D,E);
if(A){if(E.parentRelation[B]){C[E.parentRelation[B]]=A.label;
}else{C["isChildOf"]=A.label;
}}return C;
};
Exhibit.XMLImporter.configure=function(){var A={"itemTag":[],"propertyLabel":[],"itemType":[],"parentRelation":[],"propertyTags":[],"propertyNames":[]};
$("link").each(function(){if(this.hasAttribute("ex:itemTags")){A.itemTag=Exhibit.getAttribute(this,"ex:itemTags",",");
}if(this.hasAttribute("ex:propertyLabels")){A.propertyLabel=Exhibit.getAttribute(this,"ex:propertyLabels",",");
}if(this.hasAttribute("ex:itemTypes")){A.itemType=Exhibit.getAttribute(this,"ex:itemTypes",",");
}if(this.hasAttribute("ex:parentRelations")){A.parentRelation=Exhibit.getAttribute(this,"ex:parentRelations",",");
...
Exhibit.XMLImporter.determineType=function(A,B){if(B.itemTag.indexOf(A.nodeName)>=0){return"Item";
}else{if($(A).children().length==0){return"property";
}else{return"fakeItem";
}}};
Exhibit.XMLImporter.load=function(E,F,A){var B=this;
var C=typeof E=="string"?E:E.href;
C=Exhibit.Persistence.resolveURL(C);
var G=function(J,H,I){Exhibit.UI.hideBusyIndicator();
Exhibit.UI.showHelp(Exhibit.l10n.failedToLoadDataFileMessage(C));
...
var D=function(){Exhibit.UI.hideBusyIndicator();
try{var I=null;
try{xmlDoc=Exhibit.XMLImporter.getXMLDocument(C);
var J=B.configure();
I={"items":[]};
for(index in J.itemTag){I=Exhibit.XMLImporter.getItems(xmlDoc,I,index,J);
}}catch(H){Exhibit.UI.showJsonFileValidation(Exhibit.l10n.badJsonMessage(C,H),C);
}if(I!=null){F.loadData(I,Exhibit.Persistence.getBaseURL(C));
}}catch(H){SimileAjax.Debug.exception(H,"Error loading Exhibit JSON data from "+C);
}finally{if(A){A();
}}};
Exhibit.UI.showBusyIndicator();
SimileAjax.XmlHttp.get(C,G,D);
...
if(A){return A;
}else{alert("Error finding xml doc "+B);
return ;
}};
Exhibit.XMLImporter.appendUserPropertyToArray=function(C,E,A){var B=E.propertyTags.indexOf(C.nodeName);
var D=A[E.propertyNames[B]];
...
Exhibit.XMLImporter.getItems=function(D,C,B,E){var A=this;
$(E.itemTag[B],D).each(function(){var K=[];
var F=[];
$(this).children().each(function(){F.push(this);
});
objectToAppend={};
while(F.length>0){var J=F.pop();
if($(J).text().length<=0){continue;
}var G=A.determineType(J,E);
if(G=="property"){if(K.indexOf(J.nodeName)>=0){if(E.propertyTags.indexOf(J.nodeName)>=0){objectToAppend[E.propertyNames[B]]=A.appendUserPropertyToArray(J,E,objectToAppend);
}else{objectToAppend[J.nodeName]=A.appendPropertyToArray(J,E,objectToAppend);
}}else{if(E.propertyTags.indexOf(J.nodeName)>=0){var H=E.propertyTags.indexOf(J.nodeName);
objectToAppend[E.propertyNames[H]]=$(J).text();
}else{objectToAppend[J.nodeName]=$(J).text();
}}K.push(J.nodeName);
}else{if(G=="Item"){var H=E.itemTag.indexOf(J.nodeName);
var I=A.configureItem(J,{},E,H);
objectToAppend[I.type]=I.label;
}else{if(G=="fakeItem"){$(J).children().each(function(){F.push(this);
});
}else{alert("error: nodetype not understood");
}}}}objectToAppend=A.configureItem(this,objectToAppend,E,B);
C.items.push(objectToAppend);
});
return C;
};
Exhibit.XMLImporter.getParentItem=function(C,B){if(C.parentNode==null){return null;
...
Exhibit.XMLImporter.configureItem=function(D,C,E,B){if(!(C.label)&&E.propertyLabel[B]!=null){C["label"]=$(E.propertyLabel[B],D).eq(0).text();
}else{C["label"]=$(D).children().eq(0).text();
}if(!(C.type)&&E.itemType[B]!=null){C["type"]=E.itemType[B];
}else{C["type"]=D.nodeName;
}var A=this.getParentItem(D,E);
if(A){if(E.parentRelation[B]){C[E.parentRelation[B]]=A.label;
...
Exhibit.XMLImporter.configure=function(){var A={"itemTag":[],"propertyLabel":[],"itemType":[],"parentRelation":[],"propertyTags":[],"propertyNames":[]};
$("link").each(function(){A.itemTag=Exhibit.getAttribute(this,"ex:itemTags",",")||[];
A.propertyLabel=Exhibit.getAttribute(this,"ex:propertyLabels",",")||[];
A.itemType=Exhibit.getAttribute(this,"ex:itemTypes",",")||[];
A.parentRelation=Exhibit.getAttribute(this,"ex:parentRelations",",")||[];
A.propertyNames=Exhibit.getAttribute(this,"ex:propertyNames",",")||[];
A.propertyTags=Exhibit.getAttribute(this,"ex:propertyTags",",")||[];
});
return A;
};
Exhibit.XMLImporter.determineType=function(A,B){if(B.itemTag.indexOf(A.nodeName)>=0){return"Item";
...
I={"items":[]};
for(index=0;
index<J.itemTag.length;
index++){I=Exhibit.XMLImporter.getItems(xmlDoc,I,index,J);
}}catch(H){Exhibit.UI.showJsonFileValidation(Exhibit.l10n.badJsonMessage(C,H),C);
}if(I!=null){F.loadData(I,Exhibit.Persistence.getBaseURL(C));
}}catch(H){SimileAjax.Debug.exception(H,"Error loading Exhibit JSON data from "+C);
}finally{if(A){A();
...
L.visit(function(Q){J.push({id:Q,sortKey:""});
});
}if(J.length>0){var C=this._columns[this._settings.sortColumn];
J.sort(this._createSortFunction(J,C.expression,this._settings.sortAscending));
...
var M=function(Q){var R=P._columns[Q];
if(R.label==null){R.label=P._getColumnLabel(R.expression);
}var S=document.createElement("th");
Exhibit.TabularView.createColumnHeader(exhibit,S,R.label,Q==P._settings.sortColumn,P._settings.sortAscending,function(U,T,V){P._doSort(Q);
...
if(this._rowTemplate!=null){F=function(Q){var R=J[Q];
var S=Exhibit.Lens.constructFromLensTemplate(R.id,P._rowTemplate,O,P._uiContext);
if(P._settings.rowStyler!=null){P._settings.rowStyler(R.id,N,S,Q);
}};
}else{F=function(R){var T=J[R];
var U=O.insertRow(O.rows.length);
for(var X=0;
X<P._columns.length;
X++){var S=P._columns[X];
var W=U.insertCell(X);
var Q=S.expression.evaluate({"value":T.id},{"value":"item"},"value",N);
var V=S.format=="list"?Q.valueType:S.format;
S.uiContext.formatList(Q.values,Q.size,V,function(Y){W.appendChild(Y);
});
if(S.styler!=null){S.styler(T.id,N,W);
}}if(P._settings.rowStyler!=null){P._settings.rowStyler(T.id,N,U,R);
}};
}var A,E;
var B=false;
...
Exhibit.TabularView.prototype._createSortFunction=function(L,I,B){var H=this._uiContext.getDatabase();
var F=B?1:-1;
for(var Q=0;
Q<this._sortState.length;
Q++){if([I,F]==this._sortState[Q]){this._sortState=this._sortState.slice(0,Q).concat(this._sortState.slice(Q+1));
}}this._sortState.unshift([I,F]);
var A=function(V,U){return C(V,U,F*(V.sortKey-U.sortKey));
};
var G=function(V,U){return C(V,U,F*V.sortKey.localeCompare(U.sortKey));
};
var C=function(X,U,W){for(var Y=0;
W==0&&Y<X.extraSortKeys.length;
Y++){var V=R([X.extraSortKeys[Y].type],true);
X.extraSortKeys[Y].value=V.coersion(X.extraSortKeys[Y].value);
U.extraSortKeys[Y].value=V.coersion(U.extraSortKeys[Y].value);
W=V.sortingFunction(X,U,Y);
}return W;
};
var T=this;
var M=function(V,U,W){return T._sortState[W][1]*(V.extraSortKeys[W].value-U.extraSortKeys[W].value);
};
var P=function(V,U,W){return T._sortState[W][1]*V.extraSortKeys[W].value.localeCompare(U.extraSortKeys[W].value);
};
var D=[];
var N={};
for(var Q=0;
Q<L.length;
Q++){var S=L[Q];
var J=I.evaluate({"value":S.id},{"value":"item"},"value",H);
J.values.visit(function(U){S.sortKey=U;
});
for(var O=0;
O<this._sortState.length;
O++){var K=this._sortState[O][0].evaluate({"value":S.id},{"value":"item"},"value",H);
if(!S.extraSortKeys){S.extraSortKeys=[];
}K.values.visit(function(U){S.extraSortKeys.push({"value":U,"type":K.valueType});
});
}if(!(J.valueType in N)){N[J.valueType]=true;
D.push(J.valueType);
}}var R=function(Y,U){var W="text";
if(Y.length==1){W=Y[0];
}else{W="text";
}var X;
var V;
if(W=="number"){V=U?M:A;
X=function(Z){if(Z==null){return Number.NEGATIVE_INFINITY;
}else{if(typeof Z=="number"){return Z;
}else{var a=parseFloat(Z);
if(isNaN(a)){return Number.NEGATIVE_INFINITY;
}else{return a;
}}}};
}else{if(W=="date"){V=U?M:A;
X=function(Z){if(Z==null){return Number.NEGATIVE_INFINITY;
}else{if(Z instanceof Date){return Z.getTime();
}else{try{return SimileAjax.DateTime.parseIso8601DateTime(Z).getTime();
}catch(a){return Number.NEGATIVE_INFINITY;
}}}};
}else{if(W=="boolean"){V=U?M:A;
X=function(Z){if(Z==null){return Number.NEGATIVE_INFINITY;
}else{if(typeof Z=="boolean"){return Z?1:0;
}else{return Z.toString().toLowerCase()=="true";
}}};
}else{if(W=="item"){V=U?P:G;
X=function(Z){if(Z==null){return Exhibit.l10n.missingSortKey;
}else{var a=H.getObject(Z,"label");
return(a==null)?Z:a;
}};
}else{V=U?P:G;
X=function(Z){if(Z==null){return Exhibit.l10n.missingSortKey;
...
}}}}return{"sortingFunction":V,"coersion":X};
};
var E=R(D,false);
for(var Q=0;
Q<L.length;
Q++){var S=L[Q];
S.sortKey=E.coersion(S.sortKey);
}return E.sortingFunction;
};
