...
(function(){var isCompiled=("Exhibit_EditingExtension_isCompiled" in window)&&window.Exhibit_EditingExtension_isCompiled;
Exhibit.EditingExtension={params:{bundle:false}};
var javascriptFiles=["extra.js","ui/lens.js","ui/editing-lens.js","ui/editing-formatter.js","data/editing-backend.js"];
var cssFiles=["editing.css"];
...
if("Exhibit_EditingExtension_parameters" in window){SimileAjax.parseURLParameters(Exhibit_EditingExtension_parameters,Exhibit.EditingExtension.params,paramTypes);
}}else{var url=SimileAjax.findScript(document,"/editing-extension.js");
if(url==null){SimileAjax.Debug.exception(new Error("Failed to derive URL prefix for Simile Exhibit Editing Extension code files"));
return ;
...
var cssURLs=[];
if(Exhibit.EditingExtension.params.bundle){scriptURLs.push(Exhibit.EditingExtension.urlPrefix+"editing-extension-bundle.js");
cssURLs.push(Exhibit.EditingExtension.urlPrefix+"editing-extension-bundle.css");
}else{SimileAjax.prefixURLs(scriptURLs,Exhibit.EditingExtension.urlPrefix+"scripts/",javascriptFiles);
SimileAjax.prefixURLs(cssURLs,Exhibit.EditingExtension.urlPrefix+"styles/",cssFiles);
}for(var i=0;
i<Exhibit.locales.length;
i++){scriptURLs.push(Exhibit.EditingExtension.urlPrefix+"locales/"+Exhibit.locales[i]+"/editing-locale.js");
}if(!isCompiled){SimileAjax.includeJavascriptFiles(document,"",scriptURLs);
SimileAjax.includeCssFiles(document,"",cssURLs);
}})();


/* editing-extension-bundle-debug.js */
