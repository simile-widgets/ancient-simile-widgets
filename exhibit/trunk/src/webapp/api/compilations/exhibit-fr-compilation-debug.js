...
for(var i=0;
i<n;
i++){data[i]=path.rangeBackward(this._maxRange.min+i*range,this._maxRange.min+(i+1)*range,false,items,database).values.size();
...
Exhibit.SliderFacet.prototype._getMaxRange=function(){var path=this._expression.getPath();
var database=this._uiContext.getDatabase();
var propertyID=path.getLastSegment().property;
var property=database.getProperty(propertyID);
...
}if(name=="ex:onshow"){templateNode.attributes.push({name:name,value:value});
}else{if(name.length>3&&name.substr(0,3)=="ex:"){name=name.substr(3);
...
if(!settings.grouped&&settings.paginate&&pageCount>1){fromIndex=settings.page*settings.pageSize;
toIndex=Math.min(fromIndex+settings.pageSize,totalCount);
if(settings.showHeader){this._headerDom.renderPageLinks(settings.page,pageCount,settings.pageWindow);
}if(settings.showFooter){this._footerDom.renderPageLinks(settings.page,pageCount,settings.pageWindow);
...
var dom=SimileAjax.DOM.createDOMFromString(footerDiv,Exhibit.OrderedViewFrame.footerTemplate,{});
...
Exhibit.OrderedViewFrame.renderPageLinks=function(div,page,pageCount,pageWindow,gotoPage){var l10n=Exhibit.OrderedViewFrame.l10n;
div.className="exhibit-collectionView-pagingControls";
div.innerHTML="";
var self=this;
var renderPageLink=function(label,index){var a=document.createElement("a");
a.innerHTML=label;
...
var renderPageNumber=function(index){if(index==page){var elmt=document.createElement("span");
elmt.className="exhibit-collectionView-pagingControls-currentPage";
elmt.innerHTML=(index+1);
div.appendChild(elmt);
}else{renderPageLink(index+1,index);
}};
var renderHTML=function(html){var span=document.createElement("span");
span.innerHTML=html;
...
if(generatePagingControls){this._renderPagingDiv(this._dom.topPagingDiv,items.length,this._settings.page);
...
Exhibit.TabularView.prototype._renderPagingDiv=function(div,itemCount,page){var l10n=Exhibit.TabularView.l10n;
...
a.href="javascript:{}";
div.appendChild(a);
var handler=function(elmt,evt,target){self._gotoPage(index);
SimileAjax.DOM.cancelEvent(evt);
return false;
};
SimileAjax.WindowManager.registerEvent(a,"click",handler);
...
var pageWindowEnd=pageCount-1;
if(page-this._settings.pageWindow>1){renderPageNumber(0);
renderHTML(l10n.pageWindowEllipses);
pageWindowStart=page-this._settings.pageWindow;
}if(page+this._settings.pageWindow<pageCount-2){pageWindowEnd=page+this._settings.pageWindow;
}for(var i=pageWindowStart;
i<=pageWindowEnd;
i++){if(i>pageWindowStart){renderHTML(l10n.pageSeparator);
}renderPageNumber(i);
...
var headerTemplate={elmt:div,className:"exhibit-collectionView-header",children:[{tag:"div",field:"collectionSummaryDiv"},{tag:"div",className:"exhibit-tabularView-pagingControls",field:"topPagingDiv"},{tag:"div",field:"bodyDiv"},{tag:"div",className:"exhibit-tabularView-pagingControls",field:"bottomPagingDiv"}]};
...
/* locale.js */
(function(){var isCompiled=("Exhibit_isCompiled" in window)&&window.Exhibit_isCompiled;
if(!isCompiled){var javascriptFiles=["exhibit-l10n.js","data/database-l10n.js","ui/ui-context-l10n.js","ui/lens-l10n.js","ui/formatter-l10n.js","ui/widgets/collection-summary-widget-l10n.js","ui/views/view-panel-l10n.js","ui/views/ordered-view-frame-l10n.js","ui/views/tile-view-l10n.js","ui/views/thumbnail-view-l10n.js","ui/views/tabular-view-l10n.js","util/coders-l10n.js","util/facets-l10n.js","util/views-l10n.js"];
var cssFiles=[];
var urlPrefix=Exhibit.urlPrefix+"locales/fr/";
if(Exhibit.params.bundle){SimileAjax.includeJavascriptFiles(document,urlPrefix,["exhibit-fr-bundle.js"]);
if(cssFiles.length>0){SimileAjax.includeCssFiles(document,urlPrefix,["exhibit-fr-bundle.css"]);
}}else{SimileAjax.includeJavascriptFiles(document,urlPrefix+"scripts/",javascriptFiles);
