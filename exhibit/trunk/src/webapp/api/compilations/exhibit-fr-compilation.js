...
Exhibit.TsvCsvImporter.parseTsvCsv=function(T,U,N,Q){var Z=Exhibit.TsvCsvImporter.formatType(U);
var R=T.split("\n");
var O;
var P=N!=null;
var X;
var Y;
if(P){X=Exhibit.TsvCsvImporter.replaceAll(N,",",Z);
}if(P){Y=X.split(Z);
if(Q=="false"){O=R;
}else{O=R.slice(1);
}}else{if(Q=="false"){alert("No header row was given for the property names. Either specify them in the ex:properties attribute or add a header row to the file.");
return ;
}else{Y=R[0].split(Z);
O=R.slice(1);
}}while(O[O.length-1]==""){O=O.slice(0,O.length-1);
}var V=Exhibit.TsvCsvImporter.getProperties(Y);
var W=Exhibit.TsvCsvImporter.getItems(Z,O,Y);
