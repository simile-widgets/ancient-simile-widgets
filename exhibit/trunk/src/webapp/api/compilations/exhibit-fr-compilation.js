...
N._uri="http://simile.mit.edu/2006/11/exhibit#change";
N._valueType="text";
...
P._uri="http://simile.mit.edu/2006/11/exhibit#changedItem";
P._valueType="text";
...
I._uri="http://simile.mit.edu/2006/11/exhibit#modified";
I._valueType="text";
I._label="modified";
...
Exhibit.ExhibitXMLImporter.getXMLDocument=function(D){var C=null;
$.ajax({url:D,type:"GET",dataType:"xml",async:false,success:function(A){C=A;
}});
if(C){return C;
}else{alert("ERROR FINDING XML DOC");
return ;
}};
Exhibit.ExhibitXMLImporter.appendUserPropertyToArray=function(I,G,F){var J=G.propertyTags.indexOf(I.nodeName);
var H=F[G.propertyNames[J]];
if(typeof F[G.propertyNames[J]]=="string"){H=[H];
H.push(I.textContent);
}else{H.push(I.textContent);
}return H;
};
Exhibit.ExhibitXMLImporter.appendPropertyToArray=function(H,F,E){var G=E[H.nodeName];
if(typeof G=="string"){G=[G];
G.push(H.textContent);
}else{G.push(H.textContent);
}return G;
};
Exhibit.ExhibitXMLImporter.getItems=function(H,I,J,G){var F=this;
$(G.itemTag[J],H).each(function(){var A=[];
var L=[];
$(this).children().each(function(){L.push(this);
});
objectToAppend={};
while(L.length){var B=L.pop();
var E=F.determineType(B,G);
if(E=="property"){if(A.indexOf(B.nodeName)>=0){if(G.propertyTags.indexOf(B.nodeName)>=0){objectToAppend[G.propertyNames[J]]=F.appendUserPropertyToArray(B,G,objectToAppend);
}else{objectToAppend[B.nodeName]=F.appendPropertyToArray(B,G,objectToAppend);
}}else{if(G.propertyTags.indexOf(B.nodeName)>=0){var D=G.propertyTags.indexOf(B.nodeName);
objectToAppend[G.propertyNames[D]]=B.textContent;
}else{objectToAppend[B.nodeName]=B.textContent;
}}A.push(B.nodeName);
}else{if(E=="Item"){var D=G.itemTag.indexOf(B.nodeName);
var C=F.configureItem(B,{},G,D);
objectToAppend[C.type]=C.label;
}else{if(E=="fakeItem"){$(B).children().each(function(){L.push(this);
});
}else{alert("error: nodetype not understood");
}}}}objectToAppend=F.configureItem(this,objectToAppend,G,J);
I.items.push(objectToAppend);
});
return I;
};
Exhibit.ExhibitXMLImporter.getParentItem=function(E,F){if(E.parentNode==null){return null;
}else{if(F.itemTag.indexOf(E.parentNode.nodeName)>=0){var D=F.itemTag.indexOf(E.parentNode.nodeName);
return this.configureItem(E.parentNode,{},F,D);
}else{this.getParentItem(E.parentNode,F);
}}};
Exhibit.ExhibitXMLImporter.configureItem=function(H,I,G,J){if(!(I.label)&&G.propertyLabel[J]!=null){I["label"]=$(G.propertyLabel[J],H)[0].textContent;
}else{I["label"]=$(H).children()[0].textContent;
}if(!(I.type)&&G.itemType[J]!=null){I["type"]=G.itemType[J];
}else{I["type"]=H.nodeName;
}var F=this.getParentItem(H,G);
if(F){if(G.parentRelation[J]){I[G.parentRelation[J]]=F.label;
}else{I["isChildOf"]=F.label;
}}return I;
};
Exhibit.ExhibitXMLImporter.configure=function(){var B={"itemTag":[],"propertyLabel":[],"itemType":[],"parentRelation":[],"propertyTags":[],"propertyNames":[]};
$("link").each(function(){if(this.hasAttribute("ex:itemTag")){B.itemTag=Exhibit.getAttribute(this,"ex:itemTag",",");
}if(this.hasAttribute("ex:setPropertyAsLabel")){B.propertyLabel=Exhibit.getAttribute(this,"ex:setPropertyAsLabel",",");
}if(this.hasAttribute("ex:itemType")){B.itemType=Exhibit.getAttribute(this,"ex:itemType",",");
}if(this.hasAttribute("ex:parentRelation")){B.parentRelation=Exhibit.getAttribute(this,"ex:parentRelation",",");
}if(this.hasAttribute("ex:propertyNames")){B.propertyNames=Exhibit.getAttribute(this,"ex:propertyNames",",");
}if(this.hasAttribute("ex:propertyTags")){B.propertyTags=Exhibit.getAttribute(this,"ex:propertyTags",",");
}});
return B;
};
Exhibit.ExhibitXMLImporter.determineType=function(C,D){if(D.itemTag.indexOf(C.nodeName)>=0){return"Item";
}else{if($(C).children().length==0){return"property";
}else{return"fakeItem";
}}};
Exhibit.ExhibitXMLImporter.load=function(K,J,H){var N=this;
var M=typeof K=="string"?K:K.href;
M=Exhibit.Persistence.resolveURL(M);
var I=function(A,C,B){Exhibit.UI.hideBusyIndicator();
Exhibit.UI.showHelp(Exhibit.l10n.failedToLoadDataFileMessage(M));
if(H){H();
}};
var L=function(){Exhibit.UI.hideBusyIndicator();
try{var B=null;
try{xmlDoc=Exhibit.ExhibitXMLImporter.getXMLDocument(M);
var A=N.configure();
B={"items":[]};
for(index in A.itemTag){B=Exhibit.ExhibitXMLImporter.getItems(xmlDoc,B,index,A);
}}catch(C){Exhibit.UI.showJsonFileValidation(Exhibit.l10n.badJsonMessage(M,C),M);
}if(B!=null){J.loadData(B,Exhibit.Persistence.getBaseURL(M));
}}catch(C){SimileAjax.Debug.exception(C,"Error loading Exhibit JSON data from "+M);
}finally{if(H){H();
}}};
Exhibit.UI.showBusyIndicator();
SimileAjax.XmlHttp.get(M,I,L);
};
...
Exhibit.HtmlTableImporter.loadTable=function(AO,o){var w=function(A){return A.textContent||A.innerText||"";
};
var AM=function(A,C){var D={},F=false,E,G,B;
for(B=0;
E=C[B];
...
var AJ=["uri","label","pluralLabel"];
var AF=["uri","valueType","label","reverseLabel","pluralLabel","reversePluralLabel","groupingLabel","reverseGroupingLabel"];
var AP=["valueParser","arity"];
var u={};
var AD=Exhibit.getAttribute(AO,"type");
var t=AD&&AM(AO,AJ);
if(t){u.types={};
u.types[AD]=t;
}var m=[],AE={},AC=[],x,AB;
var AI,s=AO.getElementsByTagName("tr");
var AA,AG=s[0].getElementsByTagName("th");
for(AB=0;
AA=AG[AB];
AB++){var AH=w(AA).trim();
var y=false;
var l=AM(AA,AF);
var AK=Exhibit.getAttribute(AA,"name");
if(AK){l=l||{};
l.label=l.label||AH;
AH=AK;
}if(l){AE[AH]=l;
if(AE[AH].valueType=="textwithlink"){AE[AH].valueType="text";
AE[(AH+"-link")]={valueType:"url"};
y=true;
}u.properties=AE;
}m.push(AH);
l=AM(AA,AP)||{};
if(l.valueParser&&l.valueParser in window){l.valueParser=window[l.valueParser];
}else{if(l.arity=="single"){l.valueParser=function(A,C,B,D){return A.trim();
};
}else{l.valueParser=function(F,C,A,D){if(F.indexOf(";")==-1){return F.trim();
}var B=F.split(";");
for(var E=0;
E<B.length;
E++){B[E]=B[E].trim();
...
if(y){var k=l.valueParser;
l.valueParser=function(A,D,B,E){var F=D.getElementsByTagName("a");
if(!F.length){return k(A,D,B,E);
}var C={};
C[m[E]]=A.trim();
C[(m[E]+"-link")]=F[0].href;
return C;
...
}var AL,q=AO.getElementsByTagName("img");
while(AL=q[0]){AL.parentNode.replaceChild(document.createTextNode(AL.src),AL);
}var n=[],v,r;
for(x=1;
AI=s[x];
x++){var AN={};
var h=AI.getElementsByTagName("td");
for(AB=0;
v=h[AB];
AB++){var r=w(v);
data=AC[AB].valueParser(r,v,x,AB);
if(data==null||r===""){continue;
}if(typeof data=="object"&&!(data instanceof Array)){for(var z in data){AN[z]=data[z];
}}else{AN[m[AB]]=data;
}}if(AD){AN.type=AD;
}n.push(AN);
u.items=n;
}o.loadData(u,Exhibit.Persistence.resolveURL(location.href));
};
...
Exhibit.JSONPImporter.googleSpreadsheetsConverter=function(c,AB,y){var AC=";";
if((y)&&(typeof y!="string")){var o=Exhibit.getAttribute(y,"separator");
if(o!=null&&o.length>0){AC=o;
}}var m=[];
var x={};
var s={};
var u={"text":true,"number":true,"item":true,"url":true,"boolean":true,"date":true};
var q=c.feed.entry||[];
for(var h=0;
h<q.length;
h++){var AG=q[h];
var l=AG.id.$t;
var AJ=l.lastIndexOf("C");
var n=l.lastIndexOf("R");
q[h]={row:parseInt(l.substring(n+1,AJ))-1,col:parseInt(l.substring(AJ+1))-1,val:AG.content.$t};
}var v=0;
var w=function(){if(v<q.length){var B=q[v++];
var A=[B];
while(v<q.length){var C=q[v];
...
h<AE.length;
h++){var AH=AE[h];
var r=AH.val.trim().replace(/^\{/g,"").replace(/\}$/g,"").split(":");
var AD=r[0].trim();
var d=r.length>1?r[1].split(","):[];
var z={single:false};
for(var AL=0;
AL<d.length;
AL++){var AI=d[AL].trim();
if(AI in u){z.valueType=AI;
}else{if(AI=="single"){z.single=true;
}}}AK[AH.col]=AD;
x[AD]=z;
}var AA=null;
while((AA=w())!=null){var AM={};
for(var h=0;
h<AA.length;
h++){var AH=AA[h];
var AD=AK[AH.col];
if(typeof AD=="string"){var AF=/^\d{1,2}\/\d{1,2}\/\d{4}$/;
if(AF.exec(AH.val)){AH.val=Exhibit.Database.makeISO8601DateString(new Date(AH.val));
}AM[AD]=AH.val;
var z=x[AD];
if(!z.single){var k=AH.val.split(AC);
for(var t=0;
t<k.length;
t++){k[t]=k[t].trim();
}AM[AD]=k;
}else{AM[AD]=AH.val.trim();
}}}m.push(AM);
}}return{types:s,properties:x,items:m};
};
...
var removeFirstLine=Exhibit.getAttribute(link,"removeFirstLine")==true?true:false;
var fError=function(statusText,status,xmlhttp){Exhibit.UI.hideBusyIndicator();
Exhibit.UI.showHelp(Exhibit.l10n.failedToLoadDataFileMessage(url));
if(cont){cont();
}};
...
o=eval(Exhibit.TsvCsvImporter.parseTsvCsv(text,type,expressionString,removeFirstLine));
}catch(e){Exhibit.UI.showJsonFileValidation(Exhibit.l10n.badJsonMessage(url,e),url);
}if(o!=null){database.loadData(o,Exhibit.Persistence.getBaseURL(url));
...
Exhibit.TsvCsvImporter.parseTsvCsv=function(T,U,O,X){var N=Exhibit.TsvCsvImporter.formatType(U);
var R=T.split("\n");
var P=O!=null;
var Y;
if(X){R=line.slice(1);
}if(P&&N=="\t"){Y=Exhibit.TsvCsvImporter.replaceAll(O,",","\t");
}var Z=P?Y.split(N):R[0].split(N);
var Q=R.slice(1);
while(Q[Q.length-1]==""){Q=Q.slice(0,Q.length-1);
}var V=Exhibit.TsvCsvImporter.getProperties(Z);
var W=Exhibit.TsvCsvImporter.getItems(N,Q,Z);
var S={"properties":V,"items":W};
return S;
};
Exhibit.TsvCsvImporter.formatType=function(D){var C="";
if(D=="tab-separated-values"||D=="tsv"){C=C.concat("\t");
}else{if(D=="comma-separated-values"||D=="csv"){C=C.concat(",");
}else{alert("invalid format, must be tsv or csv");
}}return C;
};
Exhibit.TsvCsvImporter.getProperties=function(G){var K={};
var H={"text":true,"number":true,"item":true,"url":true,"boolean":true,"date":true};
for(i=0;
i<G.length;
i++){var I=G[i];
var J="";
if(I.match(":")){I=I.substring(0,I.lastIndexOf(":"));
var L=I.substring(I.lastIndexOf(":")+1);
if(L in H){J=L;
}else{J="text";
}}else{J="text";
}K[I]={"valueType":J};
...
Exhibit.TsvCsvImporter.getItems=function(c,Q,a){var Y=[];
var S=";";
for(i=0;
i<Q.length;
i++){var T=Q[i].split(c);
...
for(var d=0;
d<T.length;
d++){if(T[d].indexOf('"')==0&&T[d][T[d].length-1]!='"'){Z=true;
var X=d;
while(Z){joined=[T[X]+","+T[X+1]];
T=T.slice(0,X).concat(joined,T.slice(X+2));
if(T[X][T[X].length-1]=='"'){Z=false;
}}}}}if(T.length<a.length){while(T.length!=a.length){T.push("");
}}var U={};
for(var d=0;
d<a.length;
d++){var V=T[d];
V=Exhibit.TsvCsvImporter.replaceAll(V,'""','"');
if(V[0]=='"'){V=V.slice(1);
}if(V[V.length-1]=='"'){V=V.slice(0,V.length-1);
}var R=V.split(S);
if(R.length>1){for(var P=0;
P<R.length;
P++){while(R[P][0]==" "){R[P]=R[P].slice(1);
}}}var W=a[d];
var b=W.match(":")?W.substring(0,W.lastIndexOf(":")):W;
U[b]=R;
}Y.push(U);
}return Y;
};
...
k=this._rangeIndex;
countItems=function(A){return k.getSubjectsInRange(A.from,String.fromCharCode(A.to.charCodeAt(0)+1),true,null,d).size();
};
var c=[];
...
for(var Z=0;
Z<k.getCount();
Z+=1){var T=k._pairs[Z].value.substr(0,1).toUpperCase();
if(!S(T)){c.push(T);
}}for(var Y=0;
Y<c.length;
...
Exhibit.CloudFacet.prototype._computeFacet=function(g){var k=this._uiContext.getDatabase();
var l=[];
var X="text";
var h=this;
if(this._expression.isPath()){var e=this._expression.getPath();
var U=e.walkForward(g,"item",k);
X=U.valueType;
if(U.size>0){U.forEachValue(function(A){var B=e.evaluateBackward(A,X,g,k);
if(B.size>=h._settings.minimumCount||h._valueSet.contains(A)){l.push({value:A,count:B.size});
}});
}}else{this._buildMaps();
X=this._valueType;
...
for(var Z=0;
Z<d.length;
Z++){if(g.contains(d[Z])){m++;
}}if(m>=this._settings.minimumCount||this._valueSet.contains(c)){l.push({value:c,count:m});
}}}if(l.length>0){var V=this._valueSet;
var q=X=="item"?function(A){var B=k.getObject(A,"label");
return B!=null?B:A;
}:function(A){return A;
};
for(var Z=0;
Z<l.length;
Z++){var r=l[Z];
r.actionLabel=r.selectionLabel=q(r.value);
r.selected=V.contains(r.value);
}var n=function(A,B){return A.selectionLabel.localeCompare(B.selectionLabel);
};
if("_orderMap" in this){var Y=this._orderMap;
n=function(A,B){if(A.selectionLabel in Y){if(B.selectionLabel in Y){return Y[A.selectionLabel]-Y[B.selectionLabel];
}else{return -1;
}}else{if(B.selectionLabel in Y){return 1;
}else{return A.selectionLabel.localeCompare(B.selectionLabel);
}}};
}else{if(X=="number"){n=function(A,B){A=parseFloat(A.value);
...
if(this._settings.sortDirection=="reverse"){o=function(A,B){return b(B,A);
};
}l.sort(o);
}if(this._settings.showMissing||this._selectMissing){this._buildMaps();
var m=0;
for(var W in this._missingItems){if(g.contains(W)){m++;
}}if(m>0||this._selectMissing){var a=document.createElement("span");
...
l.unshift({value:null,count:m,selected:this._selectMissing,selectionLabel:a,actionLabel:Exhibit.FacetUtilities.l10n.missingThisField});
}}return l;
};
...
}}}var k={selection:V.toArray(),selectMissing:d};
var h={selection:Y.toArray(),selectMissing:a};
var W=("facetLabel" in this._settings)?this._settings.facetLabel:"";
SimileAjax.History.addLengthyAction(function(){X.applyRestrictions(k);
...
for(var l=X;
l<W;
l+=this._settings.interval){var d={from:l,to:l+this._settings.interval,selected:false};
...
var k=this._dom.valuesContainer;
...
Exhibit.FormatParser._internalParse=function(k,u,b,x){var Z=Exhibit.FormatScanner;
var q=u.token();
var c=function(){u.next();
q=u.token();
};
var r=function(){return q!=null?q.start:u.index();
};
var v=function(A,C,B){k.putSetting("format/"+A+"/"+C,B);
};
var t=function(A,B,C){if(q!=null&&q.type!=Z.IDENTIFIER&&q.value in C){v(A,B,C[q.value]);
c();
return false;
}return true;
};
var l=function(A,B,C){if(t(A,B,C)){if(q==null||q.type!=Z.NUMBER){throw new Error("Missing number at position "+r());
}v(A,B,q.value);
c();
}};
var a=function(A,B,C){if(t(A,B,C)){if(q==null||q.type!=Z.NUMBER){throw new Error("Missing integer at position "+r());
}v(A,B,Math.round(q.value));
c();
}};
var X=function(A,B,C){if(t(A,B,C)){if(q==null||q.type!=Z.NUMBER||q.value<0){throw new Error("Missing non-negative integer at position "+r());
}v(A,B,Math.round(q.value));
c();
}};
var o=function(A,B,C){if(t(A,B,C)){if(q==null||q.type!=Z.STRING){throw new Error("Missing string at position "+r());
}v(A,B,q.value);
c();
}};
var h=function(A,B,C){if(t(A,B,C)){if(q==null||q.type!=Z.URL){throw new Error("Missing url at position "+r());
}v(A,B,q.value);
c();
}};
var d=function(A,B,C){if(t(A,B,C)){if(q==null||q.type!=Z.EXPRESSION){throw new Error("Missing expression at position "+r());
}v(A,B,q.value);
c();
}};
var s=function(A,B,C){if(t(A,B,C)){if(q==null||(q.type!=Z.EXPRESSION&&q.type!=Z.STRING)){throw new Error("Missing expression or string at position "+r());
}v(A,B,q.value);
c();
}};
var e=function(A,C,B){if(q==null||q.type!=Z.IDENTIFIER){throw new Error("Missing option at position "+r());
}for(var D=0;
D<B.length;
D++){if(q.value==B[D]){v(A,C,q.value);
c();
return ;
}}throw new Error("Unsupported option "+q.value+" for setting "+C+" on value type "+A+" found at position "+r());
};
var n=function(A,B,D,E){outer:while(q!=null&&q.type==Z.IDENTIFIER){for(var C=0;
C<D.length;
C++){if(q.value==D[C]){v(A,B+"/"+q.value,true);
c();
continue outer;
}}if(q.value in E){v(A,B+"/"+E[q.value],false);
c();
continue outer;
}throw new Error("Unsupported flag "+q.value+" for setting "+B+" on value type "+A+" found at position "+r());
}};
var Y=function(A,B){switch(A){case"number":switch(B){case"decimal-digits":X(A,B,{"default":-1});
return ;
}break;
case"date":switch(B){case"time-zone":l(A,B,{"default":null});
return ;
...
case"image":switch(B){case"tooltip":s(A,B,{"none":null});
return ;
case"max-width":case"max-height":a(A,B,{"none":-1});
return ;
}break;
case"url":switch(B){case"target":o(A,B,{"none":null});
return ;
...
case"currency":switch(B){case"negative-format":n(A,B,["red","parentheses","signed"],{"unsigned":"signed","no-parenthesis":"parentheses","black":"red"});
return ;
case"symbol":o(A,B,{"default":"$","none":null});
return ;
...
case"list":switch(B){case"separator":case"last-separator":case"pair-separator":case"empty-text":o(A,B,{});
return ;
}break;
}throw new Error("Unsupported setting called "+B+" for value type "+A+" found at position "+r());
};
var g=function(A){while(q!=null&&q.type==Z.IDENTIFIER){var B=q.value;
c();
if(q==null||q.type!=Z.DELIMITER||q.value!=":"){throw new Error("Missing : at position "+r());
}c();
Y(A,B);
if(q==null||q.type!=Z.DELIMITER||q.value!=";"){break;
}else{c();
}}};
var m=function(){if(q==null||q.type!=Z.IDENTIFIER){throw new Error("Missing value type at position "+r());
}var A=q.value;
if(!(A in Exhibit.FormatParser._valueTypes)){throw new Error("Unsupported value type "+A+" at position "+r());
}c();
if(q!=null&&q.type==Z.DELIMITER&&q.value=="{"){c();
g(A);
if(q==null||q.type!=Z.DELIMITER||q.value!="}"){throw new Error("Missing } at position "+r());
}c();
}return A;
};
var w=function(){var A="text";
while(q!=null&&q.type==Z.IDENTIFIER){A=m();
}return A;
};
if(x){return w();
}else{return m();
...
Exhibit.Lens._constructFromLensTemplateNode=function(y,v,AJ,AE,s){if(typeof AJ=="string"){AE.appendChild(document.createTextNode(AJ));
return ;
}var x=AJ.uiContext;
var r=x.getDatabase();
var AA=AJ.children;
function q(){if(AA!=null){for(var A=0;
A<AA.length;
A++){Exhibit.Lens._constructFromLensTemplateNode(y,v,AA[A],w,s);
}}}if(AJ.condition!=null){if(AJ.condition.test=="if-exists"){if(!AJ.condition.expression.testExists(y,v,"value",r)){return ;
}}else{if(AJ.condition.test=="if"){if(AJ.condition.expression.evaluate(y,v,"value",r).values.contains(true)){if(AA!=null&&AA.length>0){Exhibit.Lens._constructFromLensTemplateNode(y,v,AA[0],AE,s);
}}else{if(AA!=null&&AA.length>1){Exhibit.Lens._constructFromLensTemplateNode(y,v,AA[1],AE,s);
}}return ;
}else{if(AJ.condition.test=="select"){var AF=AJ.condition.expression.evaluate(y,v,"value",r).values;
if(AA!=null){var AC=null;
for(var a=0;
a<AA.length;
a++){var u=AA[a];
if(u.condition!=null&&u.condition.test=="case"){if(AF.contains(u.condition.value)){Exhibit.Lens._constructFromLensTemplateNode(y,v,u,AE,s);
return ;
}}else{if(typeof u!="string"){AC=u;
}}}}if(AC!=null){Exhibit.Lens._constructFromLensTemplateNode(y,v,AC,AE,s);
}return ;
}}}}var w=Exhibit.Lens._constructElmtWithAttributes(AJ,AE,r);
if(AJ.contentAttributes!=null){var AM=AJ.contentAttributes;
for(var h=0;
h<AM.length;
h++){var t=AM[h];
var AF=[];
t.expression.evaluate(y,v,"value",r).values.visit(function(A){AF.push(A);
});
var l=AF.join(";");
if(t.isStyle){w.style[t.name]=l;
}else{if("class"==t.name){w.className=l;
}else{if(Exhibit.Lens._attributeValueIsSafe(t.name,l)){w.setAttribute(t.name,l);
}}}}}if(AJ.subcontentAttributes!=null){var AB=AJ.subcontentAttributes;
for(var h=0;
h<AB.length;
h++){var t=AB[h];
var AI=t.fragments;
...
o<AI.length;
o++){var AH=AI[o];
if(typeof AH=="string"){m+=AH;
}else{m+=AH.evaluateSingle(y,v,"value",r).value;
}}if(t.isStyle){w.style[t.name]=m;
}else{if("class"==t.name){w.className=m;
}else{if(Exhibit.Lens._attributeValueIsSafe(t.name,m)){w.setAttribute(t.name,m);
}}}}}if(!Exhibit.params.safe){var n=AJ.handlers;
for(var c=0;
c<n.length;
c++){var AG=n[c];
w[AG.name]=AG.code;
}}var AD=y["value"];
if(AJ.control!=null){switch(AJ.control){case"item-link":var AL=document.createElement("a");
AL.innerHTML=Exhibit.l10n.itemLinkLabel;
AL.href=Exhibit.Persistence.getItemLink(AD);
AL.target="_blank";
w.appendChild(AL);
break;
case"remove-item":if(!s.disableEditWidgets&&r.isNewItem(AD)){if(AJ.tag=="a"){w.href="javascript:";
}SimileAjax.jQuery(w).click(function(){r.removeItem(AD);
});
q();
}else{AE.removeChild(w);
}break;
case"start-editing":if(AJ.tag=="a"){w.href="javascript:";
}if(s.disableEditWidgets){AE.removeChild(w);
}else{if(s.inPopup){SimileAjax.jQuery(w).click(function(){Exhibit.UI.showItemInPopup(AD,null,x,{lensType:"edit",coords:s.coords});
});
q();
}else{SimileAjax.jQuery(w).click(function(){x.setEditMode(AD,true);
x.getCollection()._listeners.fire("onItemsChanged",[]);
});
q();
}}break;
case"stop-editing":if(AJ.tag=="a"){w.href="javascript:";
}if(s.disableEditWidgets){AE.removeChild(w);
}else{if(s.inPopup){SimileAjax.jQuery(w).click(function(){Exhibit.UI.showItemInPopup(AD,null,x,{lensType:"normal",coords:s.coords});
});
q();
}else{SimileAjax.jQuery(w).click(function(){x.setEditMode(AD,false);
x.getCollection()._listeners.fire("onItemsChanged",[]);
});
q();
}}break;
case"accept-changes":if(r.isSubmission(AD)){if(AJ.tag=="a"){w.href="javascript:";
}SimileAjax.jQuery(w).click(function(){r.mergeSubmissionIntoItem(AD);
});
q();
}else{SimileAjax.Debug.warn("accept-changes element in non-submission item");
AE.removeChild(w);
}break;
}}else{if(AJ.content!=null){var m=AJ.content.evaluate(y,v,"value",r);
if(AA!=null){var k={"value":m.valueType,"index":"number"};
var z=1;
var AK=function(C){var A={"value":C,"index":z++};
for(var B=0;
B<AA.length;
B++){Exhibit.Lens._constructFromLensTemplateNode(A,k,AA[B],w,s);
}};
if(m.values instanceof Array){for(var h=0;
h<m.values.length;
h++){AK(m.values[h]);
}}else{m.values.visit(AK);
}}else{Exhibit.Lens._constructDefaultValueList(m.values,m.valueType,w,AJ.uiContext);
}}else{if(AJ.edit!=null){q();
Exhibit.Lens._constructEditableContent(AJ,w,AD,x);
}else{if(AA!=null){for(var h=0;
h<AA.length;
h++){Exhibit.Lens._constructFromLensTemplateNode(y,v,AA[h],w,s);
}}}}}};
...
if(!g.grouped&&g.paginate&&(Q>1||(Q>0&&g.alwaysShowPagingControls))){X=g.page*g.pageSize;
e=Math.min(X+g.pageSize,V);
if(g.showHeader&&(g.pagingControlLocations=="top"||g.pagingControlLocations=="topbottom")){this._headerDom.renderPageLinks(g.page,Q,g.pageWindow);
}if(g.showFooter&&(g.pagingControlLocations=="bottom"||g.pagingControlLocations=="topbottom")){this._footerDom.renderPageLinks(g.page,Q,g.pageWindow);
...
Exhibit.OrderedViewFrame.prototype._openSortPopup=function(g,k){var c=this;
var e=this._uiContext.getDatabase();
var T=Exhibit.UI.createPopupMenuDom(g);
var b=this._getOrders();
if(k>=0){var Y=b[k];
var l=e.getProperty(Y.property);
var d=Y.forward?l.getPluralLabel():l.getReversePluralLabel();
var W=Y.forward?l.getValueType():"item";
var m=Exhibit.Database.l10n.sortLabels[W];
m=(m!=null)?m:Exhibit.Database.l10n.sortLabels["text"];
T.appendMenuItem(m.ascending,Exhibit.urlPrefix+(Y.ascending?"images/option-check.png":"images/option.png"),Y.ascending?function(){}:function(){c._reSort(k,Y.property,Y.forward,true,false);
});
T.appendMenuItem(m.descending,Exhibit.urlPrefix+(Y.ascending?"images/option.png":"images/option-check.png"),Y.ascending?function(){c._reSort(k,Y.property,Y.forward,false,false);
}:function(){});
if(b.length>1){T.appendSeparator();
T.appendMenuItem(Exhibit.OrderedViewFrame.l10n.removeOrderLabel,null,function(){c._removeOrder(k);
});
}}var o=[];
var n=this._getPossibleOrders();
for(X=0;
X<n.length;
X++){var h=n[X];
var a=false;
for(var Z=(k<0)?b.length-1:k;
Z>=0;
Z--){var V=b[Z];
if(V.property==h.property&&V.forward==h.forward){a=true;
break;
}}if(!a){var l=e.getProperty(h.property);
o.push({property:h.property,forward:h.forward,ascending:h.ascending,label:h.forward?l.getPluralLabel():l.getReversePluralLabel()});
}}if(o.length>0){if(k>=0){T.appendSeparator();
}o.sort(function(A,B){return A.label.localeCompare(B.label);
});
var U=function(A){T.appendMenuItem(A.label,null,function(){c._reSort(k,A.property,A.forward,A.ascending,true);
});
};
for(var X=0;
X<o.length;
...
var P=String.substitute(Exhibit.OrderedViewFrame.headerTemplate+"<"+O.pagingControlContainerElement+" class='exhibit-collectionView-pagingControls' style='display: none;' id='topPagingDiv'></"+O.pagingControlContainerElement+">",[O.sortingControlsTemplate]);
...
var I=SimileAjax.DOM.createDOMFromString(K,Exhibit.OrderedViewFrame.footerTemplate+"<"+M.pagingControlContainerElement+" class='exhibit-collectionView-pagingControls' style='display: none;' id='bottomPagingDiv'></"+M.pagingControlContainerElement+">",{});
...
Exhibit.OrderedViewFrame.renderPageLinks=function(N,W,O,Q,T){var R=Exhibit.OrderedViewFrame.l10n;
N.className="exhibit-collectionView-pagingControls";
N.innerHTML="";
var S=this;
var P=function(C,D){var B=document.createElement(R.pagingControlElement);
B.className="exhibit-collectionView-pagingControls-page";
N.appendChild(B);
var E=document.createElement("a");
E.innerHTML=C;
E.href="javascript:{}";
E.title=R.makePagingLinkTooltip(D);
B.appendChild(E);
var A=function(G,H,F){T(D);
SimileAjax.DOM.cancelEvent(H);
return false;
};
SimileAjax.WindowManager.registerEvent(E,"click",A);
};
var U=function(B){if(B==W){var A=document.createElement(R.pagingControlElement);
A.className="exhibit-collectionView-pagingControls-currentPage";
A.innerHTML=(B+1);
N.appendChild(A);
}else{P(B+1,B);
}};
var Y=function(A){var B=document.createElement(R.pagingControlElement);
B.innerHTML=A;
N.appendChild(B);
};
if(W>0){P(R.previousPage,W-1);
if(R.pageSeparator.length>0){Y(" ");
}}var X=0;
var V=O-1;
if(W-Q>1){U(0);
...
Z<=V;
Z++){if(Z>X&&R.pageSeparator.length>0){Y(R.pageSeparator);
}U(Z);
}if(V<O-1){Y(R.pageWindowEllipses);
U(O-1);
}if(W<O-1){if(R.pageSeparator.length>0){Y(" ");
}P(R.nextPage,W+1);
}};
...
S=(a.length>this._settings.pageSize)||(a.length>0&&this._settings.alwaysShowPagingControls);
}else{T=0;
g=a.length;
}for(var d=T;
d<g;
...
if(S){if(this._settings.pagingControlLocations=="top"||this._settings.pagingControlLocations=="topbottom"){this._renderPagingDiv(this._dom.topPagingDiv,a.length,this._settings.page);
this._dom.topPagingDiv.style.display="block";
}if(this._settings.pagingControlLocations=="bottom"||this._settings.pagingControlLocations=="topbottom"){this._renderPagingDiv(this._dom.bottomPagingDiv,a.length,this._settings.page);
...
Exhibit.TabularView.prototype._renderPagingDiv=function(G,H,I){var J=Math.ceil(H/this._settings.pageSize);
var F=this;
Exhibit.OrderedViewFrame.renderPageLinks(G,I,J,this._settings.pageWindow,function(A){F._gotoPage(A);
});
};
...
F._reconstruct();
},Exhibit.OrderedViewFrame.l10n.makePagingActionTitle(H));
};
Exhibit.TabularView._constructDefaultValueList=function(H,F,G,E){E.formatList(H,H.size(),F,function(A){G.appendChild(A);
});
};
Exhibit.TabularView.createDom=function(F){var E=Exhibit.TabularView.l10n;
var H=Exhibit.OrderedViewFrame.l10n;
var G={elmt:F,className:"exhibit-collectionView-header",children:[{tag:"div",field:"collectionSummaryDiv"},{tag:H.pagingControlContainerElement,className:"exhibit-tabularView-pagingControls",field:"topPagingDiv"},{tag:"div",field:"bodyDiv"},{tag:H.pagingControlContainerElement,className:"exhibit-tabularView-pagingControls",field:"bottomPagingDiv"}]};
return SimileAjax.DOM.createDOMFromTemplate(G);
};
...
Exhibit.ThumbnailView.prototype._reconstructWithFloats=function(){var D=this;
...
J.className=Exhibit.ThumbnailView._itemContainerClass;
...
Exhibit.ThumbnailView.prototype._reconstructWithTable=function(){var D=this;
var F={div:this._dom.bodyDiv,groupDoms:[],groupCounts:[],table:null,columnIndex:0};
var E=function(B){for(var A=B;
A<F.groupDoms.length;
A++){F.groupDoms[A].countSpan.innerHTML=F.groupCounts[A];
}F.groupDoms=F.groupDoms.slice(0,B);
...
this._orderedViewFrame.onNewItem=function(B,K){if(F.columnIndex>=D._settings.columnCount){F.columnIndex=0;
}if(F.table==null){F.table=Exhibit.ThumbnailView.constructTableItemContainer();
F.div.appendChild(F.table);
}if(F.columnIndex==0){F.table.insertRow(F.table.rows.length);
}var A=F.table.rows[F.table.rows.length-1].insertCell(F.columnIndex++);
for(var J=0;
J<F.groupCounts.length;
J++){F.groupCounts[J]++;
}var L=document.createElement("div");
L.className=Exhibit.ThumbnailView._itemContainerClass;
var C=D._lensRegistry.createLens(B,L,D._uiContext);
A.appendChild(L);
};
this._div.style.display="none";
this._dom.bodyDiv.innerHTML="";
...
Exhibit.ThumbnailView.constructTableItemContainer=function(){var B=document.createElement("table");
B.className="exhibit-thumbnailView-body";
return B;
};
...
Exhibit.LegendGradientWidget.prototype.addGradient=function(m){var AA=[];
var AA=m;
var l=function(B,A){return B.value-A.value;
};
AA.sort(l);
...
a.style.backgroundColor="rgb("+AA[c].red+","+AA[c].green+","+AA[c].blue+")";
var r=document.createElement("td");
var AB=document.createElement("div");
var u=document.createTextNode(AA[c].value);
AB.appendChild(u);
r.appendChild(AB);
...
d<Z;
d+=x){var v=(d-t)/(Z-t);
var z=Math.floor(AA[c].red+v*(AA[c+1].red-AA[c].red));
var w=Math.floor(AA[c].green+v*(AA[c+1].green-AA[c].green));
var n=Math.floor(AA[c].blue+v*(AA[c+1].blue-AA[c].blue));
var a=document.createElement("td");
a.count=o;
a.style.backgroundColor="rgb("+z+","+w+","+n+")";
var r=document.createElement("td");
var AB=document.createElement("div");
var u=document.createTextNode((Math.floor(d*100))/100);
AB.appendChild(u);
r.appendChild(AB);
AB.style.width="2px";
...
g.appendChild(r);
h.appendChild(a);
k.appendChild(document.createElement("td"));
o++;
...
var a=document.createElement("td");
a.style.backgroundColor="rgb("+AA[q].red+","+AA[q].green+","+AA[q].blue+")";
var r=document.createElement("td");
var AB=document.createElement("div");
...
var B=[];
var A=Exhibit.urlPrefix+"locales/fr/";
if(Exhibit.params.bundle){SimileAjax.includeJavascriptFiles(document,A,["exhibit-fr-bundle.js"]);
if(B.length>0){SimileAjax.includeCssFiles(document,A,["exhibit-fr-bundle.css"]);
}}else{SimileAjax.includeJavascriptFiles(document,A+"scripts/",D);
SimileAjax.includeCssFiles(document,A+"styles/",B);
}}})();


/* exhibit-fr-bundle.js */
...
Exhibit.Database.l10n.labelItemsOfType=function(L,O,K,P){var I=L==1?Exhibit.Database.l10n.itemType.label:Exhibit.Database.l10n.itemType.pluralLabel;
var M=K.getType(O);
if(M){I=M.getLabel();
if(L!=1){var J=M.getProperty("pluralLabel");
if(J){I=J;
}}}var N=document.createElement("span");
N.innerHTML="<span class='"+P+"'>"+L+"</span> "+I;
return N;
};
...
Exhibit.l10n.badJsonMessage=function(C,D){return"Le fichier de données JSON\n  "+C+"\ncontient des erreurs :\n\n"+D;
};
Exhibit.l10n.failedToLoadDataFileMessage=function(B){return"Impossible de trouver le fichier de données\n  "+B+"\nVérifiez que le nom du fichier est correct.";
};
...
Exhibit.OrderedViewFrame.l10n.formatSortActionTitle=function(D,C){return"Trier par "+D+" ("+C+")";
};
Exhibit.OrderedViewFrame.l10n.formatRemoveOrderActionTitle=function(D,C){return"Critère de tri retiré : "+D+" ("+C+")";
};
...
Exhibit.OrderedViewFrame.l10n.formatDontShowAll=function(B){return"Montrer seulement les "+B+" premiers résultats";
};
Exhibit.OrderedViewFrame.l10n.formatShowAll=function(B){return"Montrer l'ensemble des "+B+" résultats";
};
...
Exhibit.TabularView.l10n.makeSortActionTitle=function(C,D){return(D?"triés par ordre croissant selon ":"triés par ordre décroissant selon ")+C;
};
...
}Exhibit.ViewPanel.l10n.createSelectViewActionTitle=function(B){return"Sélectionner la vue "+B;
};
Exhibit.ViewPanel.l10n.missingViewClassMessage="La spécification pour une des vues ne contient pas le champ viewClass.";
Exhibit.ViewPanel.l10n.viewClassNotFunctionMessage=function(B){return"La valeur '"+B+"' de l'attribut viewClass que vous avez spécifiée\npour une des vues ne correspond pas à une fonction Javascript.";
};
Exhibit.ViewPanel.l10n.badViewClassMessage=function(B){return"La valeur '"+B+"' de l'attribut viewClass que vous avez spécifiée\npour une des vues n'est pas une expression Javascript valide.";
};
...
return String.substitute("<a class='exhibit-action exhibit-views-unplottableCount' href='javascript:void' id='unplottableCountLink'>%0</a> sur <class class='exhibit-views-totalCount'>%1</span> ne peuvent pas être tracés.",[F==1?(F+" résultat"):(F+" résultats"),H]);
};
