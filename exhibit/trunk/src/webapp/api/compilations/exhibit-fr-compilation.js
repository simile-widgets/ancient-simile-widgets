...
}if(this._settings.showFooter){this._footerDom.setCounts(R,this._settings.abbreviatedCount,this._settings.showAll,!(V&&this._grouped));
}};
Exhibit.OrderedViewFrame.prototype._internalReconstruct=function(U){var Q=this;
var M=this._settings;
var R=this._uiContext.getDatabase();
var X=this._getOrders();
var O=0;
var T=false;
var W=function(A){if((T&&M.grouped)||M.showAll||O<M.abbreviatedCount){Q.onNewItem(A,O++);
}};
var S=function(B,A,C){if((T&&M.grouped)||M.showAll||O<M.abbreviatedCount){Q.onNewGroup(B,A,C);
}};
var P=function(E,G){var J=X[G];
var B=J.forward?R.getObjectsUnion(E,J.property):R.getSubjectsUnion(E,J.property);
var H="text";
if(J.forward){var C=R.getProperty(J.property);
H=C!=null?C.getValueType():"text";
}else{H="item";
}var A=(H=="item"||H=="text")?N(E,G,B,H):V(E,G,B,H);
var F=false;
for(var I=0;
I<A.length;
...
I<A.length;
I++){var D=A[I];
if(D.items.size()>0){if(F&&M.grouped){S(D.display,H,G);
}E.removeSet(D.items);
if(D.items.size()>1&&G<X.length-1){P(D.items,G+1);
}else{D.items.visit(W);
}}}if(E.size()>0){if(F&&M.grouped){S(Exhibit.l10n.missingSortKey,H,G);
}if(E.size()>1&&G<X.length-1){P(E,G+1);
}else{E.visit(W);
}}};
var N=function(F,G,C,H){var B=[];
var E;
var A;
var J=X[G];
if(H=="item"){C.visit(function(L){var K=R.getObject(L,"label");
K=K!=null?K:L;
B.push({itemID:L,display:K});
});
E=function(Z,K){var L=Z.display.localeCompare(K.display);
return L!=0?L:Z.itemID.localeCompare(K.itemID);
};
A=J.forward?function(K){return R.getSubjects(K.itemID,J.property,null,F);
}:function(K){return R.getObjects(K.itemID,J.property,null,F);
};
}else{C.visit(function(K){B.push({display:K});
});
E=function(L,K){return L.display.localeCompare(K.display);
};
A=J.forward?function(K){return R.getSubjects(K.display,J.property,null,F);
}:function(K){return R.getObjects(K.display,J.property,null,F);
};
}B.sort(function(L,K){return(J.ascending?1:-1)*E(L,K);
});
for(var I=0;
I<B.length;
I++){var D=B[I];
D.items=A(D);
if(!M.showDuplicates){F.removeSet(D.items);
}}return B;
};
var V=function(F,H,B,I){var A=[];
var G={};
var K=X[H];
var E;
if(I=="number"){E=function(Z){if(typeof Z=="number"){return Z;
}else{try{return parseFloat(Z);
}catch(L){return null;
}}};
}else{E=function(Z){if(Z instanceof Date){return Z.getTime();
}else{try{return SimileAjax.DateTime.parseIso8601DateTime(Z.toString()).getTime();
}catch(L){return null;
}}};
}B.visit(function(L){var a=E(L);
if(a!=null){var b=G[a];
if(!b){b={sortkey:a,display:L,values:[],items:new Exhibit.Set()};
G[a]=b;
A.push(b);
}b.values.push(L);
}});
A.sort(function(L,Z){return(K.ascending?1:-1)*(L.sortkey-Z.sortkey);
});
for(var J=0;
J<A.length;
J++){var C=A[J];
...
for(var D=0;
D<B.length;
D++){if(K.forward){R.getSubjects(B[D],K.property,C.items,F);
}else{R.getObjects(B[D],K.property,C.items,F);
}}if(!M.showDuplicates){F.removeSet(C.items);
}}return A;
};
P(U,0);
return T;
...
Exhibit.OrderedViewFrame.createHeaderDom=function(J,P,Q,K,R,M){var N=Exhibit.OrderedViewFrame.l10n;
var O=String.substitute(Exhibit.OrderedViewFrame.headerTemplate,[N.sortingControlsTemplate]);
var L=SimileAjax.DOM.createDOMFromString(P,O,{});
P.className="exhibit-collectionView-header";
if(Q){L.collectionSummaryDiv.style.display="block";
L.collectionSummaryWidget=Exhibit.CollectionSummaryWidget.create({},L.collectionSummaryDiv,J);
}if(K){L.controlsDiv.style.display="block";
L.groupOptionWidget=Exhibit.OptionWidget.create({label:N.groupedAsSortedOptionLabel,onToggle:M},L.groupOption,J);
SimileAjax.WindowManager.registerEvent(L.thenSortByAction,"click",R);
L.enableThenByAction=function(A){Exhibit.UI.enableActionLink(L.thenSortByAction,A);
};
L.setOrders=function(A){L.ordersSpan.innerHTML="";
var C=Exhibit.Formatter.createListDelimiter(L.ordersSpan,A.length,J);
for(var B=0;
B<A.length;
B++){C();
L.ordersSpan.appendChild(A[B]);
}C();
};
}L.dispose=function(){if("collectionSummaryWidget" in L){L.collectionSummaryWidget.dispose();
L.collectionSummaryWidget=null;
}L.groupOptionWidget.dispose();
L.groupOptionWidget=null;
};
return L;
};
Exhibit.OrderedViewFrame.footerTemplate="<span id='showAllSpan'></span>";
Exhibit.OrderedViewFrame.createFooterDom=function(G,I,J,L){var K=Exhibit.OrderedViewFrame.l10n;
var H=SimileAjax.DOM.createDOMFromString(I,Exhibit.OrderedViewFrame.footerTemplate,{});
I.className="exhibit-collectionView-footer";
H.setCounts=function(B,C,D,A){H.showAllSpan.innerHTML="";
if(A&&B>C){if(D){H.showAllSpan.appendChild(Exhibit.UI.makeActionLink(K.formatDontShowAll(C),L));
}else{H.showAllSpan.appendChild(Exhibit.UI.makeActionLink(K.formatShowAll(B),J));
}}};
H.dispose=function(){};
return H;
};
...
Exhibit.TabularView.prototype._reconstruct=function(){var R=this;
var Z=this._uiContext.getCollection();
var T=this._uiContext.getDatabase();
var P=this._dom.bodyDiv;
P.innerHTML="";
var X=[];
var Y=Z.countAllItems();
if(Y>0){var V=Z.getRestrictedItems();
V.visit(function(A){X.push({id:A,sortKey:""});
});
}if(X.length>0){var Q=this._columns[this._settings.sortColumn];
X.sort(this._createSortFunction(X,Q.expression,this._settings.sortAscending));
var S=document.createElement("table");
S.className="exhibit-tabularView-body";
if(this._settings.tableStyler!=null){this._settings.tableStyler(S,T);
}else{S.cellSpacing=this._settings.cellSpacing;
S.cellPadding=this._settings.cellPadding;
S.border=this._settings.border;
}var W=S.insertRow(0);
var U=function(C){var B=R._columns[C];
if(B.label==null){B.label=R._getColumnLabel(B.expression);
}var A=document.createElement("th");
Exhibit.TabularView.createColumnHeader(exhibit,A,B.label,C==R._settings.sortColumn,R._settings.sortAscending,function(E,F,D){R._doSort(C);
SimileAjax.DOM.cancelEvent(F);
return false;
});
W.appendChild(A);
...
if(this._rowTemplate!=null){O=function(C){var B=X[C];
var A=Exhibit.Lens.constructFromLensTemplate(B.id,R._rowTemplate,S,R._uiContext);
if(R._settings.rowStyler!=null){R._settings.rowStyler(B.id,T,A,C);
}};
}else{O=function(G){var E=X[G];
var D=S.insertRow(S.rows.length);
for(var A=0;
A<R._columns.length;
A++){var F=R._columns[A];
var B=D.insertCell(A);
var H=F.expression.evaluate({"value":E.id},{"value":"item"},"value",T);
var C=F.format=="list"?H.valueType:F.format;
F.uiContext.formatList(H.values,H.size,C,function(I){B.appendChild(I);
});
if(F.styler!=null){F.styler(E.id,T,B);
}}if(R._settings.rowStyler!=null){R._settings.rowStyler(E.id,T,D,G);
}};
}for(var N=0;
N<X.length;
N++){O(N);
...
Exhibit.TabularView.prototype._doSort=function(J){var N=this._settings.sortColumn;
var I=this._settings.sortAscending;
var K=J;
var H=N==K?!I:true;
var L=this._settings;
var M=this;
SimileAjax.History.addLengthyAction(function(){L.sortColumn=K;
L.sortAscending=H;
M._reconstruct();
},function(){L.sortColumn=N;
L.sortAscending=I;
M._reconstruct();
},Exhibit.TabularView.l10n.makeSortActionTitle(this._columns[J].label,H));
};
...
var F={elmt:E,className:"exhibit-collectionView-header",children:[{tag:"div",field:"collectionSummaryDiv"},{tag:"div",field:"bodyDiv"}]};
