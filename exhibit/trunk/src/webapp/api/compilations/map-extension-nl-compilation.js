...
Exhibit.MapView.prototype._rePlotItems=function(s){var t=this;
var v=this._uiContext.getCollection();
var w=this._uiContext.getDatabase();
var AL=this._settings;
var AJ=this._accessors;
var AF=v.getRestrictedItems();
var AK={};
var AC=(this._accessors.getColorKey!=null);
var j=(this._accessors.getSizeKey!=null);
var AE=(this._accessors.getIconKey!=null);
var q=(this._accessors.getIcon!=null);
AF.visit(function(H){var I=[];
t._getLatlng(H,w,function(J){if(J!=null&&"lat" in J&&"lng" in J){I.push(J);
}});
if(I.length>0){var B=null;
if(AC){B=new Exhibit.Set();
AJ.getColorKey(H,w,function(J){B.add(J);
});
}var E=null;
if(j){E=new Exhibit.Set();
AJ.getSizeKey(H,w,function(J){E.add(J);
});
}var C=null;
if(AE){C=new Exhibit.Set();
AJ.getIconKey(H,w,function(J){C.add(J);
});
}for(var F=0;
F<I.length;
F++){var G=I[F];
var A=G.lat+","+G.lng;
if(A in AK){var D=AK[A];
D.items.push(H);
if(AC){D.colorKeys.addSet(B);
}if(j){D.sizeKeys.addSet(E);
}if(AE){D.iconKeys.addSet(C);
}}else{var D={latlng:G,items:[H]};
if(AC){D.colorKeys=B;
}if(j){D.sizeKeys=E;
}if(AE){D.iconKeys=C;
}AK[A]=D;
...
var p={mixed:false,missing:false,others:false,keys:new Exhibit.Set()};
var r={mixed:false,missing:false,others:false,keys:new Exhibit.Set()};
var AB,AD=Infinity;
var o=function(F){var I=F.items.length;
if(!AB){AB=new GLatLngBounds();
}var E=t._settings.shape;
var H=t._settings.color;
if(AC){H=t._colorCoder.translateSet(F.colorKeys,z);
}var D=t._settings.iconSize;
if(j){D=t._sizeCoder.translateSet(F.sizeKeys,p);
}var C=null;
if(I==1){if(q){AJ.getIcon(F.items[0],w,function(J){C=J;
});
}}if(AE){C=t._iconCoder.translateSet(F.iconKeys,r);
}var C=Exhibit.MapView._makeIcon(E,H,D,I==1?"":I.toString(),C,t._settings);
var A=new GLatLng(F.latlng.lat,F.latlng.lng);
var G=new GMarker(A,C);
if(AD>F.latlng.maxAutoZoom){AD=F.latlng.maxAutoZoom;
}AB.extend(A);
GEvent.addListener(G,"click",function(){G.openInfoWindow(t._createInfoWindow(F.items));
if(t._selectListener!=null){t._selectListener.fire({itemIDs:F.items});
}});
t._map.addOverlay(G);
for(var B=0;
B<F.items.length;
B++){t._itemIDToMarker[F.items[B]]=G;
}};
for(var AG in AK){o(AK[AG]);
}if(AC){var AP=this._dom.legendWidget;
var i=this._colorCoder;
var x=z.keys.toArray().sort();
if(AL.colorLegendLabel!==null){AP.addLegendLabel(AL.colorLegendLabel,"color");
}if(i._gradientPoints!=null){var y=this._dom.legendWidget;
y.addGradient(this._colorCoder._gradientPoints);
}else{for(var m=0;
m<x.length;
m++){var AI=x[m];
var n=i.translate(AI);
AP.addEntry(n,AI);
}}if(z.others){AP.addEntry(i.getOthersColor(),i.getOthersLabel());
}if(z.mixed){AP.addEntry(i.getMixedColor(),i.getMixedLabel());
}if(z.missing){AP.addEntry(i.getMissingColor(),i.getMissingLabel());
}}if(j){var AP=this._dom.legendWidget;
var AM=this._sizeCoder;
var x=p.keys.toArray().sort();
if(AL.sizeLegendLabel!==null){AP.addLegendLabel(AL.sizeLegendLabel,"size");
}if(AM._gradientPoints!=null){var k=AM._gradientPoints;
var AN=(k[k.length-1].value-k[0].value)/5;
x=[];
for(var l=0;
l<6;
l++){x.push(Math.floor(k[0].value+AN*l));
}for(var m=0;
m<x.length;
m++){var AI=x[m];
var u=AM.translate(AI);
AP.addEntry(u,AI,"size");
}}else{for(var m=0;
m<x.length;
m++){var AI=x[m];
var u=AM.translate(AI);
AP.addEntry(u,AI,"size");
}if(p.others){AP.addEntry(AM.getOthersSize(),AM.getOthersLabel(),"size");
}if(p.mixed){AP.addEntry(AM.getMixedSize(),AM.getMixedLabel(),"size");
}if(p.missing){AP.addEntry(AM.getMissingSize(),AM.getMissingLabel(),"size");
}}}if(AE){var AP=this._dom.legendWidget;
var AA=this._iconCoder;
var x=r.keys.toArray().sort();
if(AL.iconLegendLabel!=null){AP.addLegendLabel(AL.iconLegendLabel,"icon");
}for(var m=0;
m<x.length;
m++){var AI=x[m];
var AO=AA.translate(AI);
AP.addEntry(AO,AI,"icon");
}if(r.others){AP.addEntry(AA.getOthersIcon(),AA.getOthersLabel(),"icon");
}if(r.mixed){AP.addEntry(AA.getMixedIcon(),AA.getMixedLabel(),"icon");
}if(r.missing){AP.addEntry(AA.getMissingIcon(),AA.getMissingLabel(),"icon");
}}if(AB&&typeof AL.zoom=="undefined"){var AH=Math.max(0,t._map.getBoundsZoomLevel(AB)-1);
AH=Math.min(AH,AD,AL.maxAutoZoom);
t._map.setZoom(AH);
}if(AB&&typeof AL.center=="undefined"){t._map.setCenter(AB.getCenter());
}};
