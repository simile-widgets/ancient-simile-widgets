...
Exhibit.XMLImporter.OldgetItems=function(xmlDoc,object,index,configuration){var self=this;
...
if(nodeType=="property"){var propertyName=configuration.propertyNames[nodeName]||nodeName;
if(propertyList.indexOf(node.nodeName)>=0){objectToAppend[propertyName]=self.appendUserPropertyToArray(node,configuration,objectToAppend);
}else{objectToAppend[propertyName]=SimileAjax.jQuery(node).text();
...
var visit=function(node,parentItem,parentProperty){var tag=node.tagName;
...
if(children.length==0){item.label=jQ.text();
}else{children.each(function(){visit(this,item,null);
});
if(configuration.labelProperty[tag]!="label"){var label=item[configuration.labelProperty[tag]]||[];
if(label.length>0){item.label=label[0];
}}if(parentItem){if(parentItem[parentProperty]){parentItem[parentProperty].push(item.label);
}else{parentItem[parentProperty]=[item.label];
}}}}else{if(children.length==0){if(parentItem&&(jQ.text().length>=0)){var property=configuration.propertyNames[tag]||tag;
if(parentItem[tag]){parentItem[tag].push(jQ.text());
}else{parentItem[configuration.propertyNames[tag]||tag]=[jQ.text()];
...
var url=typeof link=="string"?link:link.href;
var configuration;
try{configuration=self.configure(link);
url=Exhibit.Persistence.resolveURL(url);
}catch(e){SimileAjax.Debug.exception(e,"Error configuring XML importer for "+url);
return ;
}var fError=function(xmlhttp,statusText,error){Exhibit.UI.hideBusyIndicator();
if(statusText=="parsererror"){Exhibit.UI.showHelp("Invalid XML at "+url);
}else{Exhibit.UI.showHelp(Exhibit.l10n.failedToLoadDataFileMessage(url));
}if(cont){cont();
}};
var fDone=function(xmlDoc){try{var o=Exhibit.XMLImporter.getItems(xmlDoc,configuration);
database.loadData({items:o},Exhibit.Persistence.getBaseURL(url));
}catch(e){SimileAjax.Debug.exception(e,"Error loading data from "+url);
}finally{Exhibit.UI.hideBusyIndicator();
if(cont){cont();
}}};
Exhibit.UI.showBusyIndicator();
SimileAjax.jQuery.ajax({url:url,type:"GET",dataType:"xml",async:false,success:fDone,error:fError});
};
