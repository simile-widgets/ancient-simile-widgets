...
Exhibit.Database._Impl.prototype.loadDataLinks=function(fDone){var links=SimileAjax.jQuery("link[rel=exhibit/data]").get();
...
Exhibit.Database._Impl.defaultFetch=function(link,database,cont,parser){var url=typeof link=="string"?link:link.href;
...
if(importer){if(importer.parse){(importer.fetch||Exhibit.Database._Impl.defaultFetch)(link,database,fNext,importer.parse);
}else{importer.load(link,database,fNext);
}return ;
...
Exhibit.TsvImporter.parse=function(link,database,content,url){return Exhibit.TsvCsvImporter.parse(link,database,content,url,"\t");
};
Exhibit.CsvImporter.parse=function(link,database,content,url){return Exhibit.TsvCsvImporter.parse(link,database,content,url,",");
};
Exhibit.TsvCsvImporter.parse=function(link,database,content,url,separator){var url=link;
...
try{o=Exhibit.TsvCsvImporter._parseInternal(content,separator,expressionString,hasColumnTitles);
}catch(e){SimileAjax.Debug.exception(e,"Error parsing tsv/csv from "+url);
}return o;
};
Exhibit.TsvCsvImporter._parseInternal=function(text,separator,expressionString,hasColumnTitles){var data=Exhibit.TsvCsvImporter.CsvToArray(text,separator);
var exprs=null;
var propNames=[];
var properties=[];
...
for(j=0;
j<len;
j++){item[propNames[j]]=row[j];
}items.push(item);
}return{items:items,properties:properties};
};
Exhibit.TsvCsvImporter.CsvToArray=function(text,separator){var i;
if(text.indexOf('"')<0){var lines=text.split("\n");
for(i=1;
i<lines.length;
i++){lines[i]=lines[i].split(separator);
}return lines;
}var lines=text.match(/(([^\n"]+?)|("[^"]*?"))+?(?=\n|$)/g);
var records=[];
for(i=0;
i<lines.length;
i++){var pieces=lines[i].split('"');
var fields=pieces[0].split(separator);
var j=1;
while(j<pieces.length){var quoted=[pieces[j++]];
while(pieces[j].length==0){j=j+1;
quoted.push(pieces[j++]);
}quoted=quoted.join('"');
...
Exhibit.TsvCsvImporter._parseInternalOld=function(text,separator,expressionString,hasColumnTitles){var lines=text.split("\n");
var rest;
...
function maybeAdd(item,property,value){if(item&&property.length>0&&value.length>0){if(item[property]){item[property].push(value);
...
parentItem=item;
}else{if(children.length==0){if((parentItem!=null)&&(jQ.text().length>=0)){var property=configuration.propertyNames[tag]||tag;
