...
var M={id:B,label:K,change:"added",type:I,vals:{}};
...
}for(var B in this._originalValues){if(B in this._newItems){continue;
}var I=this.getObject(B,"type");
var K=this.getObject(B,"label")||B;
var M={id:B,label:K,change:"modified",type:I,vals:{}};
...
G.push({id:B,label:K,change:"deleted",type:I});
}return G;
};
...
break;
case"lens":case"edit-lens":lensElmts.push(elmt);
break;
...
}else{if(B.getAttribute("ex:role")=="lens"){this._typeToLens[A]=B.cloneNode(true);
}else{if(B.getAttribute("ex:role")=="edit-lens"){this._editLensTemplates[A]=B.cloneNode(true);
}else{SimileAjax.Debug.log("Unknown lens type ",B);
}}}};
...
L.appendChild(b);
break;
case"remove-item":if(e.tag=="a"){L.href="javascript:";
...
var F=function(){I.editItem(A,G,this.value);
};
...
/* scraper.js */
Exhibit.Scraper={};
Exhibit.Scraper.createScraperFromDOM=function(B,A){var C=Exhibit.extractOptionsFromElement(B);
return Exhibit.Scraper.createScraper(C,B,A);
};
Exhibit.Scraper.createScraper=function(E,C,B){if(C.nodeName.toLowerCase()=="a"){C.href="javascript:";
}var A=document.getElementById(E.scraperinput);
var D=function(){};
SimileAjax.jQuery(A).click(D);
};
