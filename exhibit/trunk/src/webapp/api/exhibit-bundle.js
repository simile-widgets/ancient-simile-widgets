...
Exhibit.XMLImporter.OldgetItems=function(D,C,B,E){var A=this;
SimileAjax.jQuery(E.itemTag[B],D).each(function(){var L=[];
var F=[];
SimileAjax.jQuery(this).children().each(function(){F.push(this);
});
objectToAppend={};
while(F.length>0){var K=F.pop();
if(SimileAjax.jQuery(K).text().length<=0){continue;
}var G=A.determineType(K,E);
if(G=="property"){var H=E.propertyNames[nodeName]||nodeName;
if(L.indexOf(K.nodeName)>=0){objectToAppend[H]=A.appendUserPropertyToArray(K,E,objectToAppend);
}else{objectToAppend[H]=SimileAjax.jQuery(K).text();
}L.push(K.nodeName);
}else{if(G=="Item"){var I=E.itemTag.indexOf(K.nodeName);
var J=A.configureItem(K,{},E,I);
objectToAppend[J.type]=J.label;
}else{if(G=="fakeItem"){SimileAjax.jQuery(K).children().each(function(){F.push(this);
});
...
Exhibit.XMLImporter.getItems=function(C,D){var A=[];
var B=function(H,G,E){var M=H.tagName;
var I=SimileAjax.jQuery(H);
var F=I.children();
if(M in D.itemType){var L={type:D.itemType[M]};
A.push(L);
E=D.parentRelation[M]||E;
if(F.length==0){L.label=I.text();
}else{F.each(function(){B(this,L,null);
});
if(D.labelProperty[M]!="label"){var J=L[D.labelProperty[M]]||[];
if(J.length>0){L.label=J[0];
}}if(G){if(G[E]){G[E].push(L.label);
}else{G[E]=[L.label];
}}}}else{if(F.length==0){if(G&&(I.text().length>=0)){var K=D.propertyNames[M]||M;
if(G[M]){G[M].push(I.text());
}else{G[D.propertyNames[M]||M]=[I.text()];
}}}else{F.each(function(){B(this,G,M);
});
}}};
B(C,null,null);
return A;
