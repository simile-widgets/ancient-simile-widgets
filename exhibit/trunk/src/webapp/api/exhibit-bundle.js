...
}}else{var F=Exhibit.Collection.createFromDOM(G,C,E);
}};
...
Exhibit.Database._Impl.prototype.acceptChanges=function(A){if(!this.isSubmission(A)){throw A+" is not a submission!";
}var B=this.getObject(A,"change");
if(B=="modification"){}else{if(B=="addition"){delete this._submissionRegistry[A];
this._newItems[A]=true;
}else{throw"unknown change type "+B;
}}this._listeners.fire("onAfterLoadingItems",[]);
};
...
}var C=B.getAttribute("ex:role").replace("/^exhibit-/");
