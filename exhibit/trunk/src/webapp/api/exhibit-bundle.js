...
this._dom.setSelectionCount(this.hasRestrictions(),D.size());
return D;
}};
...
Exhibit.LensRegistry.prototype.createLens=function(E,F,B){var C=new Exhibit.Lens();
var A=this.isBeingEdited(E);
var D=A?this.getEditLens(E,B.getDatabase()):this.getLens(E,B.getDatabase());
if(D==null){if(A){C._constructDefaultEditingUI(E,F,B);
}else{C._constructDefaultUI(E,F,B);
}}else{if(typeof D=="string"){C._constructFromLensTemplateURL(E,F,B,D);
}else{C._constructFromLensTemplateDOM(E,F,B,D);
}}return C;
};
Exhibit.LensRegistry.prototype.setEditMode=function(A,B){if(B){Exhibit.LensRegistry.EditModeRegistry[A]=true;
}else{delete Exhibit.LensRegistry.EditModeRegistry[A];
}};
...
Exhibit.Lens._constructFromLensTemplateNode=function(J,M,d,D){if(typeof d=="string"){D.appendChild(document.createTextNode(d));
return ;
}var K=d.uiContext;
var P=K.getDatabase();
var H=d.children;
if(d.condition!=null){if(d.condition.test=="if-exists"){if(!d.condition.expression.testExists(J,M,"value",P)){return ;
}}else{if(d.condition.test=="if"){if(d.condition.expression.evaluate(J,M,"value",P).values.contains(true)){if(H!=null&&H.length>0){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[0],D);
}}else{if(H!=null&&H.length>1){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[1],D);
}}return ;
}else{if(d.condition.test=="select"){var C=d.condition.expression.evaluate(J,M,"value",P).values;
...
}}}}var L=Exhibit.Lens._constructElmtWithAttributes(d,D,P);
if(d.contentAttributes!=null){var Y=d.contentAttributes;
for(var V=0;
V<Y.length;
V++){var O=Y[V];
var C=[];
...
}}}}}if(d.subcontentAttributes!=null){var G=d.subcontentAttributes;
for(var V=0;
V<G.length;
V++){var O=G[V];
var e=O.fragments;
...
}else{S+=A.evaluateSingle(J,M,"value",P).value;
}}if(O.isStyle){L.style[O.name]=S;
}else{if("class"==O.name){L.className=S;
}else{if(Exhibit.Lens._attributeValueIsSafe(O.name,S)){L.setAttribute(O.name,S);
}}}}}if(!Exhibit.params.safe){var R=d.handlers;
for(var W=0;
W<R.length;
W++){var B=R[W];
L[B.name]=B.code;
}}var E=J["value"];
if(d.control!=null){switch(d.control){case"item-link":var Z=document.createElement("a");
Z.innerHTML=Exhibit.l10n.itemLinkLabel;
Z.href=Exhibit.Persistence.getItemLink(E);
Z.target="_blank";
L.appendChild(Z);
break;
case"start-editing":if(d.tag=="a"){L.href="javascript:";
}SimileAjax.jQuery(L).click(function(){K.getLensRegistry().setEditMode(E,true);
K.getCollection()._listeners.fire("onItemsChanged",[]);
});
if(H!=null){for(var V=0;
V<H.length;
V++){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[V],L);
}}break;
case"stop-editing":if((typeof E)!="string"){D.removeChild(L);
}else{if(d.tag=="a"){L.href="javascript:";
}SimileAjax.jQuery(L).click(function(){K.getLensRegistry().setEditMode(E,false);
K.getCollection()._listeners.fire("onItemsChanged",[]);
});
if(H!=null){for(var V=0;
V<H.length;
...
}}else{if(d.content!=null){var S=d.content.evaluate(J,M,"value",P);
if(H!=null){var U={"value":S.valueType,"index":"number"};
var I=1;
var b=function(c){var a={"value":c,"index":I++};
for(var f=0;
f<H.length;
f++){Exhibit.Lens._constructFromLensTemplateNode(a,U,H[f],L);
}};
if(S.values instanceof Array){for(var V=0;
V<S.values.length;
V++){b(S.values[V]);
}}else{S.values.visit(b);
}}else{Exhibit.Lens._constructDefaultValueList(S.values,S.valueType,L,d.uiContext);
}}else{if(d.edit!=null){if(H!=null){for(var V=0;
V<H.length;
V++){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[V],L);
}}Exhibit.Lens._constructEditableContent(d,L,E,K);
}else{if(H!=null){for(var V=0;
V<H.length;
V++){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[V],L);
}}}}}};
...
var F=function(){var J=I.getObject(A,G);
I.removeObjects(A,G);
I.addStatement(A,G,this.value);
I.getProperty(G)._onNewData();
I._listeners.fire("onItemModified",[A,G,J,this.value]);
D.getExhibit().getDefaultCollection()._update();
};
...
case"item-creator":return Exhibit.UI.createItemCreator(D,B,A);
...
case"item-creator":return Exhibit.UI.createItemCreatorFromDOM(B,A);
case"hiddenContent":B.style.display="none";
return null;
}return null;
};
...
Exhibit.UI.createItemCreator=function(C,B,D){var H=D.getDatabase();
var G=C.itemType||"item";
var F=H.getType(G).getLabel();
var E=SimileAjax.jQuery;
var I=function(){var J="Untitled "+F;
var L="";
var K=J;
...
if(B.nodeName.toLowerCase()=="a"){B.href="javascript:";
}var A=function(){var J=I();
var U={type:G,id:J,label:J};
var L=E("<div>");
...
var T=D.getLensRegistry()._typeToLens[U.type+"-editor"];
var O=new Exhibit.Lens();
var S={type:"temporaryItem",values:U};
O._constructFromLensTemplateDOM(S,N.get(0),D,T);
L.attr("exibit_inpop",true);
var R=function(){SimileAjax.WindowManager._layers.forEach(function(V){if(E(V.elmt).find("[exibit_inpop]").length>0){SimileAjax.WindowManager.popLayer(V);
}});
};
var P=function(){R();
H.loadItems([U],"");
...
K.click(R);
M.click(P);
var Q=SimileAjax.DOM.getPageCoordinates(B);
SimileAjax.Graphics.createBubbleForContentAndPoint(L.get(0),Q.left+Math.round(B.offsetWidth/2),Q.top+Math.round(B.offsetHeight/2),D.getSetting("bubbleWidth"));
};
SimileAjax.jQuery(B).click(A);
return B;
};
Exhibit.UI.createItemCreatorFromDOM=function(B,A){var D={};
var C=Exhibit.getAttribute(B,"itemType");
if(C){D.itemType=C;
}return Exhibit.UI.createItemCreator(D,B,A);
};
