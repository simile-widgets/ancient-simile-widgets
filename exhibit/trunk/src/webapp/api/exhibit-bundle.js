...
D.addSet(C.rangeBackward(min,max.setUTCDate(max.getUTCDate()+1),A,B).values);
...
return E.rangeBackward(C,A,B,D).count>0;
};
...
for(var C=0;
C<G;
C++){E[C]=F.rangeBackward(this._maxRange.min+C*A,this._maxRange.min+(C+1)*A,B,D).values.size();
...
var B=this._uiContext.getDatabase();
return C.rangeBackward(this._range.min,this._range.max,A,B).values;
};
