...
if(!E.grouped&&E.paginate&&D>1){M=E.page*E.pageSize;
F=Math.min(M+E.pageSize,O);
if(E.showHeader){this._headerDom.renderPageLinks(E.page,D,E.pageWindow);
}if(E.showFooter){this._footerDom.renderPageLinks(E.page,D,E.pageWindow);
...
var I=String.substitute(Exhibit.OrderedViewFrame.headerTemplate,[J.sortingControlsTemplate]);
...
var G=SimileAjax.DOM.createDOMFromString(E,Exhibit.OrderedViewFrame.footerTemplate,{});
...
Exhibit.OrderedViewFrame.renderPageLinks=function(B,H,D,A,K){var M=Exhibit.OrderedViewFrame.l10n;
B.className="exhibit-collectionView-pagingControls";
B.innerHTML="";
var L=this;
var C=function(P,O){var N=document.createElement("a");
N.innerHTML=P;
N.className="exhibit-collectionView-pagingControls-page";
N.href="javascript:{}";
B.appendChild(N);
var Q=function(S,R,T){K(O);
SimileAjax.DOM.cancelEvent(R);
return false;
};
SimileAjax.WindowManager.registerEvent(N,"click",Q);
};
var J=function(N){if(N==H){var O=document.createElement("span");
O.className="exhibit-collectionView-pagingControls-currentPage";
O.innerHTML=(N+1);
B.appendChild(O);
}else{C(N+1,N);
}};
var F=function(N){var O=document.createElement("span");
O.innerHTML=N;
B.appendChild(O);
};
if(H>0){C(M.previousPage,H-1);
F(" ");
}var G=0;
var I=D-1;
if(H-A>1){J(0);
...
E<=I;
E++){if(E>G){F(M.pageSeparator);
}J(E);
}if(I<D-1){F(M.pageWindowEllipses);
J(D-1);
}if(H<D-1){F(" ");
C(M.nextPage,H+1);
}};
...
B=J.length>this._settings.pageSize;
}else{A=0;
E=J.length;
}for(var G=A;
G<E;
...
if(B){this._renderPagingDiv(this._dom.topPagingDiv,J.length,this._settings.page);
this._renderPagingDiv(this._dom.bottomPagingDiv,J.length,this._settings.page);
...
Exhibit.TabularView.prototype._renderPagingDiv=function(B,A,H){var L=Exhibit.TabularView.l10n;
var D=Math.ceil(A/this._settings.pageSize);
B.className="exhibit-tabularView-pagingControls";
B.innerHTML="";
var K=this;
var C=function(O,N){var M=document.createElement("a");
M.innerHTML=O;
M.className="exhibit-tabularView-pagingControls-page";
M.href="javascript:{}";
B.appendChild(M);
var P=function(R,Q,S){K._gotoPage(N);
SimileAjax.DOM.cancelEvent(Q);
return false;
};
SimileAjax.WindowManager.registerEvent(M,"click",P);
};
var J=function(M){if(M==H){var N=document.createElement("span");
N.className="exhibit-tabularView-pagingControls-currentPage";
N.innerHTML=(M+1);
B.appendChild(N);
}else{C(M+1,M);
}};
var F=function(M){var N=document.createElement("span");
N.innerHTML=M;
B.appendChild(N);
};
if(H>0){C(L.previousPage,H-1);
F(" ");
}var G=0;
var I=D-1;
if(H-this._settings.pageWindow>1){J(0);
F(L.pageWindowEllipses);
G=H-this._settings.pageWindow;
}if(H+this._settings.pageWindow<D-2){I=H+this._settings.pageWindow;
}for(var E=G;
E<=I;
E++){if(E>G){F(L.pageSeparator);
}J(E);
}if(I<D-1){F(L.pageWindowEllipses);
J(D-1);
}if(H<D-1){F(" ");
C(L.nextPage,H+1);
}};
...
A._reconstruct();
},Exhibit.TabularView.l10n.makePagingActionTitle(D));
};
...
var B={elmt:C,className:"exhibit-collectionView-header",children:[{tag:"div",field:"collectionSummaryDiv"},{tag:"div",className:"exhibit-tabularView-pagingControls",field:"topPagingDiv"},{tag:"div",field:"bodyDiv"},{tag:"div",className:"exhibit-tabularView-pagingControls",field:"bottomPagingDiv"}]};
