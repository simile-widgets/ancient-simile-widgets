...
Exhibit.LensRegistry.prototype.setEditMode=function(A,B){if(B){this._editModeRegistry[A]=true;
}else{delete this._editModeRegistry[A];
}};
Exhibit.LensRegistry.prototype.isBeingEdited=function(A){return this._editModeRegistry[A];
};
...
}else{if(C=="editablecontent"){D.editableContent=G;
...
case"stop-editing":if(d.tag=="a"){L.href="javascript:";
...
}}else{if(d.editableContent!=null){if(H!=null){for(var V=0;
V<H.length;
V++){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[V],L);
...
var G=B.editableContent;
var H=I.getObject(A,G);
...
case"itemCreator":return Exhibit.UI.createItemCreator(D,B,A);
...
case"itemCreator":return Exhibit.UI.createItemCreatorFromDOM(B,A);
case"hiddenContent":B.style.display="none";
return null;
}return null;
};
...
Exhibit.UI.createItemCreator=function(H,C,B){var A=B.getDatabase();
var G=H.itemType||"item";
var E=A.getType(G).getLabel();
var F=function(){var I="Untitled "+E;
var K="";
var J=I;
while(A.containsItem(J)){K++;
J=I+" "+K;
}return J;
};
if(C.nodeName.toLowerCase()=="a"){C.href="javascript:";
}var D=function(){var J=F();
var I={type:G,id:J,label:J};
A.loadItems([I],"");
A._listeners.fire("onItemAdded",[J,G]);
};
SimileAjax.jQuery(C).click(D);
return C;
};
