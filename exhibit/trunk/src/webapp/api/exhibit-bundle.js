...
var C=function(G,F){origX=G.screenX;
origLeft=parseInt(F.div.style.left);
min=F.min();
max=F.max();
return function(H){H=H||window.event;
dx=H.screenX-origX;
newLeft=origLeft+dx;
if(newLeft<min){newLeft=min;
}if(newLeft>max){newLeft=max;
}F.div.style.left=newLeft+"px";
setTimeout(function(){F.setDisplay(newLeft);
},0);
};
};
var E=function(F,G){return function(H){if(removeEventListener){document.removeEventListener("mousemove",G,false);
document.removeEventListener("mouseup",arguments.callee,false);
}F.value=F.display.innerHTML;
B._notifyFacet();
};
};
var D=function(F){if(addEventListener){F.div.addEventListener("mousedown",A(F),false);
}};
D(this._minSlider);
D(this._maxSlider);
};
