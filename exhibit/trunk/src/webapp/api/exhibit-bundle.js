...
Exhibit.Lens._constructFromLensTemplateNode=function(J,M,f,D){if(typeof f=="string"){D.appendChild(document.createTextNode(f));
return ;
}var K=f.uiContext;
var Q=K.getDatabase();
var H=f.children;
...
}}else{if(f.condition.test=="if"){if(f.condition.expression.evaluate(J,M,"value",Q).values.contains(true)){if(H!=null&&H.length>0){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[0],D);
...
L.appendChild(d);
break;
case"remove-item":if(Q.isNewItem(E)){if(f.tag=="a"){L.href="javascript:";
...
}else{if(f.tag=="a"){L.href="javascript:";
}if(K.inPopupMode()){var P=K.getPopupFunc();
SimileAjax.jQuery(L).click(function(){var a=K.isBeingEdited(E);
K.setEditMode(E,false);
P();
K.setEditMode(E,a);
});
...
}SimileAjax.jQuery(L).click(function(){Q.acceptChanges(E);
});
R();
...
for(var h=0;
h<H.length;
h++){Exhibit.Lens._constructFromLensTemplateNode(a,W,H[h],L);
}};
...
X<H.length;
X++){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[X],L);
}}}}}};
...
Exhibit.UIContext.prototype.getPopupFunc=function(){return this._popupFunc;
};
Exhibit.UIContext.prototype.enablePopupMode=function(A){this._popupFunc=A;
};
Exhibit.UIContext.prototype.disablePopupMode=function(){this._popupFunc=null;
};
...
Exhibit.UI.showItemInPopup=function(F,C,B){SimileAjax.WindowManager.popAllLayers();
var E=SimileAjax.DOM.getPageCoordinates(C);
var A=document.createElement("div");
var G=function(){Exhibit.UI.showItemInPopup(F,C,B);
};
B.enablePopupMode(G);
