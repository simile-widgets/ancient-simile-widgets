...
Exhibit.SliderFacet._settingsSpecs={"facetLabel":{type:"text"},"scroll":{type:"boolean",defaultValue:true},"height":{type:"text"},"precision":{type:"float",defaultValue:1}};
...
Exhibit.SliderFacet.prototype.update=function(A){};
...
this._facet=B;
this._prec=A||0.1;
this._range=B._maxRange;
this._scaleFactor=null;
this._minSlider={};
this._maxSlider={};
this._dom=SimileAjax.DOM.createDOMFromString(C,'<div class="exhibit-slider-bar" id="bar"><div class="exhibit-slider-handle" id="minHandle"></div><div class="exhibit-slider-handle" id="maxHandle"></div></div><div><span id="minDisplay"></span> - <span id="maxDisplay"></span></div>');
...
Exhibit.SliderFacet.slider.prototype._setSlider=function(A,B){if(B>=this._range.max){A.div.style.left=A.max;
}if(B<=this._range.min){A.div.style.left=A.min;
}A.div.style.left=B/this._scaleFactor+"px";
if(A==this._minSlider){this._range.min=B;
}else{this._range.max=B;
}this._notifyFacet();
};
...
C.offset=C.div.offsetWidth/2;
C.min=function(){return -C.offset;
};
C.max=function(){return parseInt(A.div.style.left)+A.offset-C.offset;
};
var A=this._maxSlider;
A.value=this._range.max;
A.div=this._dom.maxHandle;
A.display=this._dom.maxDisplay;
A.offset=A.div.offsetWidth/2+C.div.offsetWidth;
A.min=function(){return C.div.offsetLeft+C.offset-B._dom.bar.offsetLeft-A.offset;
};
...
Exhibit.SliderFacet.slider.prototype._registerDragging=function(){var A=function(E){return function(F){F=F||window.event;
onMove=B(F,E);
if(addEventListener){document.addEventListener("mousemove",onMove,false);
document.addEventListener("mouseup",D(E,onMove),false);
}};
};
var B=function(F,E){origX=F.screenX;
origLeft=parseInt(E.div.style.left);
min=E.min();
max=E.max();
return function(G){G=G||window.event;
dx=G.screenX-origX;
newLeft=origLeft+dx;
if(newLeft<min){newLeft=min;
}if(newLeft>max){newLeft=max;
}E.div.style.left=newLeft+"px";
E.setDisplay(newLeft);
};
};
var D=function(E,F){return function(G){if(removeEventListener){document.removeEventListener("mousemove",F,false);
document.removeEventListener("mouseup",arguments.callee,false);
}};
};
var C=function(E){if(addEventListener){E.div.addEventListener("mousedown",A(E),false);
}};
C(this._minSlider);
C(this._maxSlider);
};
...
SimileAjax.WindowManager.registerForDragging(this._dom.maxHandle,{onDragStart:function(){var D=B._dom.minHandle;
var E=B._dom.bar;
this._maxHandle=B._dom.maxHandle;
this._offset=this._maxHandle.offsetWidth/2+D.offsetWidth;
this._left=parseInt(this._maxHandle.style.left);
this._min=D.offsetLeft+D.offsetWidth/2-E.offsetLeft-this._offset;
this._max=E.offsetWidth-this._offset;
},onDragBy:function(E,D){this._left+=E;
if(this._left<this._min){this._left=this._min;
}else{if(this._left>this._max){this._left=this._max;
}}this._maxHandle.style.left=this._left+"px";
B._dom.maxDisplay.innerHTML=Exhibit.Util.round((this._left+this._offset)*B._scaleFactor+B._range.min,B._prec);
},onDragEnd:function(){B._max=B._dom.maxDisplay.innerHTML;
B._notifyFacet();
}});
};
Exhibit.SliderFacet.slider.prototype._notifyFacet=function(){this._facet.changeRange({min:this._min,max:this._max});
};
