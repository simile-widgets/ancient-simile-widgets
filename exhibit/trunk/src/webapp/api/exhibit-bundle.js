...
Exhibit.Database._Impl.prototype.editItem=function(E,D,B){if(D.toLowerCase()=="id"){throw"can't change ID of "+E;
}var C=this.getObject(E,D);
if(C==B){return ;
}this._originalValues[E]=this._originalValues[E]||{};
this._originalValues[E][D]=this._originalValues[E][D]||C;
this.removeObjects(E,D);
this.addStatement(E,D,B);
var A=this.getProperty(D);
if(A){A._onNewData();
}else{SimileAjax.Debug.warn("No property object for "+D);
}this._listeners.fire("onAfterLoadingItems",[]);
};
...
Exhibit.Database._Impl.prototype.collectChanges=function(E){E=E||Exhibit.Database.defaultIgnoredSubmissionProperties;
var G=[];
...
Exhibit.Database._Impl.prototype.acceptChanges=function(D){var B=this;
if(!this.isSubmission(D)){throw D+" is not a submission!";
}var E=this.getObject(D,"change");
if(E=="modification"){var A=this.getObject(D,"changedItem");
var C=this._spo[D];
...
var A=F.edit;
if(G.type=="temporaryItem"){var H=G.values[A]||"";
var D=function(){G.values[A]=this.value;
};
}else{var H=B.getObject(G,A);
...
var J=E.getDatabase().getAllItems();
var G=L.evaluate({"value":J},{value:"item"},"value",E.getDatabase());
var K=G.values.toArray().sort();
for(var F in K){var H=C.options.length;
var M=K[F];
var I=new Option(M,M);
C.options[H]=I;
}}if(!D){var H=C.options.length;
C.options[H]=new Option("","",true);
