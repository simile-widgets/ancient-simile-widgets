...
Exhibit.TsvCsvImporter.parseTsvCsv=function(K,J,C,G){var D=Exhibit.TsvCsvImporter.formatType(J);
var M=K.split("\n");
var B=C!=null;
var F;
if(G){M=line.slice(1);
}if(B&&D=="\t"){F=Exhibit.TsvCsvImporter.replaceAll(C,",","\t");
}var E=B?F.split(D):M[0].split(D);
var A=M.slice(1);
while(A[A.length-1]==""){A=A.slice(0,A.length-1);
}var I=Exhibit.TsvCsvImporter.getProperties(E);
var H=Exhibit.TsvCsvImporter.getItems(D,A,E);
...
i++){var E=A[i];
var D="";
if(E.match(":")){E=E.substring(0,E.lastIndexOf(":"));
var B=E.substring(E.lastIndexOf(":")+1);
if(B in F){D=B;
}else{D="text";
}}else{D="text";
}C[E]={"valueType":D};
