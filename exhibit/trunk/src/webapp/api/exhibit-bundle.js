...
var A=new Exhibit.Database._Property("modified",this);
A._valueType="text";
A._label="modified";
...
var B=this.getProperty(E);
if(B){B._onNewData();
}else{SimileAjax.Debug.warn("No property object for "+E);
}this._listeners.fire("onAfterLoadingItems",[]);
};
...
this._newItems={};
for(var A in this._items){this.removeObjects(A,"modified");
...
var H=[];
for(var B in this._newItems){var J=this.getObject(B,"type");
var L=this.getObject(B,"label")||B;
var N={id:B,label:L,changeType:"added",type:J,vals:{}};
var I=this.getAllProperties();
for(var G in I){var A=I[G];
if(E.indexOf(A)!=-1){continue;
}var D=this.getObject(B,A);
if(D){N.vals[A]={newVal:D};
}}H.push(N);
}for(var B in this._originalValues){if(B in this._newItems||this.isSubmission(B)){continue;
}var J=this.getObject(B,"type");
var L=this.getObject(B,"label")||B;
var N={id:B,label:L,changeType:"modified",type:J,vals:{}};
var K=this._originalValues[B];
var F=false;
for(var A in K){if(E.indexOf(A)!=-1){continue;
}F=true;
var M=this._originalValues[B][A];
var C=this.getObject(B,A);
if(!C){SimileAjax.Debug.warn("empty value for "+B+", "+A);
}else{N.vals[A]={oldVal:M,newVal:C};
}}if(F){H.push(N);
}}return H;
};
Exhibit.Database._Impl.prototype.commitChangeToItem=function(D){var B=this;
...
}SimileAjax.jQuery(L).click(function(){Q.commitChangeToItem(E);
});
R();
