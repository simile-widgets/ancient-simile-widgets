...
Exhibit.SliderFacet._settingsSpecs={"facetLabel":{type:"text"},"scroll":{type:"boolean",defaultValue:true},"height":{type:"text"},"precision":{type:"float",defaultValue:1},"histogram":{type:"boolean",defaultValue:true}};
...
Exhibit.SliderFacet._configure=function(D,C){Exhibit.SettingsUtilities.collectSettings(C,Exhibit.SliderFacet._settingsSpecs,D._settings);
if("expression" in C){D._expression=Exhibit.ExpressionParser.parse(C.expression);
}if(!("facetLabel" in D._settings)){D._settings.facetLabel="missing ex:facetLabel";
if(D._expression!=null&&D._expression.isPath()){var A=D._expression.getPath().getLastSegment();
var B=D._uiContext.getDatabase().getProperty(A.property);
if(B!=null){D._settings.facetLabel=A.forward?B.getLabel():B.getReverseLabel();
}}}D._maxRange=D._getMaxRange();
};
...
for(var C=0;
C<G;
C++){E[C]=F.rangeBackward(C*A,(C+1)*A,B,D).values.size();
...
this._slider1={};
this._slider2={};
this._dom=SimileAjax.DOM.createDOMFromString(C,'<div class="exhibit-slider-bar" id="bar"><div class="exhibit-slider-handle" id="slider1"></div><div class="exhibit-slider-handle" id="slider2"></div>'+(this._facet._settings.histogram?'<canvas class="exhibit-slider-histogram" id="histogram"></canvas>':"")+'</div><div class="exhibit-slider-display"><span id="display1"></span> - <span id="display2"></span></div>');
...
if(this._dom.histogram){this._dom.histogram.width=this._dom.bar.offsetWidth;
this._dom.histogram.height=this._dom.bar.offsetHeight;
}else{this._dom.bar.style.borderTop="1px solid black";
}};
Exhibit.SliderFacet.slider.prototype.resetSliders=function(){this._setSlider(this._minSlider,this._range.min);
this._setSlider(this._maxSlider,this._range.max);
};
Exhibit.SliderFacet.slider.prototype._setSlider=function(A,B){if(B>this._range.max){B=this._range.max;
}else{if(B<this._range.min){B=this._range.min;
}}B=Exhibit.Util.round(B,this._prec);
A.value=B;
A.div.style.left=(B/this._scaleFactor-A.offset)+"px";
A.display.innerHTML=B;
};
Exhibit.SliderFacet.slider.slider=function(D,C,B){var A=B._dom.bar;
this.div=D;
this.div.style.backgroundImage='url("'+Exhibit.urlPrefix+'images/slider-handle.png")';
this.display=C;
this.offset=(this.div.offsetLeft-A.offsetLeft)+this.div.offsetWidth/2;
this.min=-this.offset;
this.max=A.offsetWidth-this.offset;
this.setDisplay=function(F){var E=parseInt(this.div.style.left)+this.offset;
this.display.innerHTML=Exhibit.Util.round(E*B._scaleFactor+B._range.min,B.prec);
};
if(B._facet._settings.histogram){this.div.style.top=this.div.offsetHeight+"px";
}};
Exhibit.SliderFacet.slider.prototype._initSliders=function(){this._dom.minHandle.style.left=0;
this._dom.maxHandle.style.left=0;
var B=this;
var C=this._minSlider;
C.value=this._range.min;
C.div=this._dom.minHandle;
C.div.style.backgroundImage='url("'+Exhibit.urlPrefix+'images/right-arrow.png")';
C.display=this._dom.minDisplay;
C.offset=C.div.offsetWidth;
C.min=function(){return -C.offset;
};
C.max=function(){return parseFloat(A.div.style.left)+A.offset-C.offset;
};
if(!this._facet._settings.histogram){C.div.style.top=-C.div.offsetHeight/2+"px";
}var A=this._maxSlider;
A.value=this._range.max;
A.div=this._dom.maxHandle;
A.div.style.backgroundImage='url("'+Exhibit.urlPrefix+'images/left-arrow.png")';
A.display=this._dom.maxDisplay;
A.offset=C.div.offsetWidth;
A.min=function(){return parseFloat(C.div.style.left)+C.offset-A.offset;
};
A.max=function(){return B._dom.bar.offsetWidth-A.offset;
};
if(!this._facet._settings.histogram){A.div.style.top=-A.div.offsetHeight/2+"px";
}C.setDisplay=A.setDisplay=function(D){this.display.innerHTML=Exhibit.Util.round((D+this.offset)*B._scaleFactor+B._range.min,B._prec);
};
this.resetSliders();
};
...
document.attachEvent("onmouseup",E(F,H));
}else{document.addEventListener("mousemove",H,false);
document.addEventListener("mouseup",E(F,H),false);
}SimileAjax.DOM.cancelEvent(G);
return false;
};
};
var C=function(J,H){var K=J.screenX;
var I=parseInt(H.div.style.left);
var G=H.min;
var F=H.max;
return function(N){N=N||window.event;
var L=N.screenX-K;
var M=I+L;
if(M<G){M=G;
}if(M>F){M=F;
}H.div.style.left=M+"px";
setTimeout(function(){H.setDisplay();
},0);
};
};
var E=function(F,G){return function(H){if(document.detachEvent){document.detachEvent("onmousemove",G);
document.detachEvent("onmouseup",arguments.callee);
}else{document.removeEventListener("mousemove",G,false);
document.removeEventListener("mouseup",arguments.callee,false);
}F.value=F.display.innerHTML;
B._notifyFacet();
};
};
var D=function(F){if(document.attachEvent){F.div.attachEvent("onmousedown",A(F));
}else{F.div.addEventListener("mousedown",A(F),false);
}};
D(this._slider1);
D(this._slider2);
};
Exhibit.SliderFacet.slider.prototype._registerDragging2=function(){var B=this;
var A=function(D){return function(){this.left=parseInt(D.div.style.left);
this.min=D.min();
this.max=D.max();
};
};
var C=function(D){return function(F,E){this.left+=F;
left=this.left;
if(this.left<this.min){this.left=this.min;
}if(this.left>this.max){this.left=this.max;
}D.div.style.left=this.left+"px";
setTimeout(function(){D.setDisplay(left);
},0);
};
};
SimileAjax.WindowManager.registerForDragging(this._dom.minHandle,{onDragStart:A(this._minSlider),onDragBy:C(this._minSlider),onDragEnd:function(){B._min=B._dom.minDisplay.innerHTML;
B._notifyFacet();
}});
SimileAjax.WindowManager.registerForDragging(this._dom.maxHandle,{onDragStart:A(this._maxSlider),onDragBy:C(this._maxSlider),onDragEnd:function(){B._min=B._minSlider.display.innerHTML;
B._notifyFacet();
}});
};
...
Exhibit.SliderFacet.slider.prototype.updateHistogram=function(D,B){var B=B?B:75;
var F=this._dom.histogram;
var J=F.getContext("2d");
J.fillStyle="rgba(0,0,0,.25)";
J.clearRect(0,0,F.width,F.height);
var I=Math.max.apply(Math,D);
if(!I){return ;
}var A=this._dom.bar.offsetWidth/B;
var G=this._dom.bar.offsetHeight;
var E=G/I;
for(var C=0;
C<B;
C++){var H=Math.round(D[C]*E);
J.fillRect(C*A,G-H,A,H);
}};
