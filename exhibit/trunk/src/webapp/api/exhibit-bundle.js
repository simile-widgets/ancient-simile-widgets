...
Exhibit.ExhibitXMLImporter.getXMLDocument=function(B){var A=null;
...
Exhibit.ExhibitXMLImporter.appendUserPropertyToArray=function(C,E,A){var B=E.propertyTags.indexOf(C.nodeName);
...
Exhibit.ExhibitXMLImporter.appendPropertyToArray=function(B,D,A){var C=A[B.nodeName];
if(typeof C=="string"){C=[C];
C.push(B.textContent);
}else{C.push(B.textContent);
}return C;
};
Exhibit.ExhibitXMLImporter.getItems=function(D,C,B,E){var A=this;
$(E.itemTag[B],D).each(function(){var K=[];
var F=[];
$(this).children().each(function(){F.push(this);
});
objectToAppend={};
while(F.length){var J=F.pop();
...
objectToAppend[E.propertyNames[H]]=J.textContent;
}else{objectToAppend[J.nodeName]=J.textContent;
}}K.push(J.nodeName);
...
Exhibit.ExhibitXMLImporter.getParentItem=function(C,B){if(C.parentNode==null){return null;
...
Exhibit.ExhibitXMLImporter.configureItem=function(D,C,E,B){if(!(C.label)&&E.propertyLabel[B]!=null){C["label"]=$(E.propertyLabel[B],D)[0].textContent;
...
Exhibit.ExhibitXMLImporter.configure=function(){var A={"itemTag":[],"propertyLabel":[],"itemType":[],"parentRelation":[],"propertyTags":[],"propertyNames":[]};
...
Exhibit.ExhibitXMLImporter.determineType=function(A,B){if(B.itemTag.indexOf(A.nodeName)>=0){return"Item";
}else{if($(A).children().length==0){return"property";
}else{return"fakeItem";
}}};
Exhibit.ExhibitXMLImporter.load=function(E,F,A){var B=this;
...
try{var I=null;
try{xmlDoc=Exhibit.ExhibitXMLImporter.getXMLDocument(C);
var J=B.configure();
I={"items":[]};
for(index in J.itemTag){I=Exhibit.ExhibitXMLImporter.getItems(xmlDoc,I,index,J);
