...
var E=function(I){var H=function(K,J,L){B._filter(I.value,I.actionLabel,!(J.ctrlKey||J.metaKey));
SimileAjax.DOM.cancelEvent(J);
return false;
};
var G=document.createElement("img");
G.src=I.image;
G.className=I.selected?"exhibit-imageFacet-value exhibit-imageFacet-value-selected":"exhibit-imageFacet-value";
G.title=I.count+" "+I.tooltip;
SimileAjax.WindowManager.registerEvent(G,"click",H,SimileAjax.WindowManager.getBaseLayer());
F.appendChild(G);
};
for(var C=0;
C<A.length;
...
Exhibit.OrderedDictionary.prototype.get=function(A,B){if(!this._values[A]&&B){this.put(A,B);
}return this._values[A];
};
Exhibit.OrderedDictionary.prototype.values=function(){return this._keyOrder.map(this.get,this);
};
Exhibit.OrderedDictionary.prototype.forEach=function(A){this.values().map(A);
};
...
A(C.values().length,0);
C.put("foo","bar");
C.get("default","default value");
C.put("foo","baz");
A(C.get("foo"),"baz");
A(C.get("default"),"default value");
var B=C.values();
A(C.size(),2);
A(B.length,2);
A(B[0],"baz");
