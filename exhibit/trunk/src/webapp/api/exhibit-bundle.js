...
SimileAjax.jQuery.ajax({url:B,type:"GET",dataType:"xml",async:false,success:function(C){A=C;
}});
if(A){return A;
...
D.push(SimileAjax.jQuery(C).text());
}else{D.push(SimileAjax.jQuery(C).text());
}return D;
};
Exhibit.XMLImporter.appendPropertyToArray=function(B,D,A){var C=A[B.nodeName];
if(typeof C=="string"){C=[C];
C.push(SimileAjax.jQuery(B).text());
}else{C.push(SimileAjax.jQuery(B).text());
}return C;
};
Exhibit.XMLImporter.getItems=function(D,C,B,E){var A=this;
SimileAjax.jQuery(E.itemTag[B],D).each(function(){var K=[];
var F=[];
SimileAjax.jQuery(this).children().each(function(){F.push(this);
});
objectToAppend={};
while(F.length>0){var J=F.pop();
if(SimileAjax.jQuery(J).text().length<=0){continue;
...
objectToAppend[E.propertyNames[H]]=SimileAjax.jQuery(J).text();
}else{objectToAppend[J.nodeName]=SimileAjax.jQuery(J).text();
}}K.push(J.nodeName);
}else{if(G=="Item"){var H=E.itemTag.indexOf(J.nodeName);
var I=A.configureItem(J,{},E,H);
objectToAppend[I.type]=I.label;
}else{if(G=="fakeItem"){SimileAjax.jQuery(J).children().each(function(){F.push(this);
});
...
Exhibit.XMLImporter.configureItem=function(D,C,E,B){if(!(C.label)&&E.labelTag[B]!=null){C["label"]=SimileAjax.jQuery(E.labelTag[B],D).eq(0).text();
}else{C["label"]=SimileAjax.jQuery(D).children().eq(0).text();
...
Exhibit.XMLImporter.configure=function(A){var B={"itemTag":[],"labelTag":[],"itemType":[],"parentRelation":[],"propertyTags":[],"propertyNames":[]};
B.itemTag=Exhibit.getAttribute(A,"ex:itemTags",",")||[];
B.labelTag=Exhibit.getAttribute(A,"ex:labelTags",",")||[];
B.itemType=Exhibit.getAttribute(A,"ex:itemTypes",",")||[];
B.parentRelation=Exhibit.getAttribute(A,"ex:parentRelations",",")||[];
B.propertyNames=Exhibit.getAttribute(A,"ex:propertyNames",",")||[];
B.propertyTags=Exhibit.getAttribute(A,"ex:propertyTags",",")||[];
return B;
};
Exhibit.XMLImporter.determineType=function(A,B){if(B.itemTag.indexOf(A.nodeName)>=0){return"Item";
}else{if(SimileAjax.jQuery(A).children().length==0){return"property";
...
var V;
if(W=="number"){V=U?M:A;
X=function(Z){if(Z==null){return Number.MAX_VALUE;
}else{if(typeof Z=="number"){return Z;
}else{var a=parseFloat(Z);
if(isNaN(a)){return Number.MAX_VALUE;
}else{return a;
}}}};
...
}catch(a){return Number.MAX_VALUE;
}}}};
