...
var D=Exhibit.Database.l10n;
var F=new Exhibit.Database._Type("Item");
F._custom=Exhibit.Database.l10n.itemType;
this._types["Item"]=F;
var E=new Exhibit.Database._Property("label",this);
E._uri="http://www.w3.org/2000/01/rdf-schema#label";
E._valueType="text";
E._label=D.labelProperty.label;
E._pluralLabel=D.labelProperty.pluralLabel;
E._reverseLabel=D.labelProperty.reverseLabel;
E._reversePluralLabel=D.labelProperty.reversePluralLabel;
E._groupingLabel=D.labelProperty.groupingLabel;
E._reverseGroupingLabel=D.labelProperty.reverseGroupingLabel;
this._properties["label"]=E;
var G=new Exhibit.Database._Property("type");
G._uri="http://www.w3.org/1999/02/22-rdf-syntax-ns#type";
G._valueType="text";
G._label="type";
G._pluralLabel=D.typeProperty.label;
G._reverseLabel=D.typeProperty.reverseLabel;
G._reversePluralLabel=D.typeProperty.reversePluralLabel;
G._groupingLabel=D.typeProperty.groupingLabel;
G._reverseGroupingLabel=D.typeProperty.reverseGroupingLabel;
this._properties["type"]=G;
var B=new Exhibit.Database._Property("uri");
B._uri="http://simile.mit.edu/2006/11/exhibit#uri";
B._valueType="url";
B._label="URI";
B._pluralLabel="URIs";
B._reverseLabel="URI of";
B._reversePluralLabel="URIs of";
B._groupingLabel="URIs";
B._reverseGroupingLabel="things named by these URIs";
this._properties["uri"]=B;
var C=new Exhibit.Database._Property("change",this);
C._valueType="text";
C._label="change type";
C._pluralLabel="change types";
C._reverseLabel="change type of";
C._reversePluralLabel="change types of";
C._groupingLabel="change types";
C._reverseGroupingLabel="changes of this type";
this._properties["change"]=C;
var A=new Exhibit.Database._Property("changedItem",this);
A._valueType="text";
A._label="changed item";
A._pluralLabel="changed item";
A._groupingLabel="changed items";
this._properties["changedItem"]=A;
};
...
Exhibit.Database._Impl.prototype.setEditMode=function(A,B){if(B){this._editRegistry[A]=true;
}else{delete this._editRegistry[A];
}};
Exhibit.Database._Impl.prototype.isBeingEdited=function(A){return !!this._editRegistry[A];
};
...
this.setEditMode(A.id,true);
delete this._deletedItems[A.id];
this._newItems[A.id]=true;
this._listeners.fire("onAfterLoadingItems",[]);
};
Exhibit.Database._Impl.prototype.editItem=function(D,C,A){if(C.toLowerCase()=="id"){throw"can't change ID of "+D;
}var B=this.getObject(D,C);
this._originalValues[D]=this._originalValues[D]||{};
this._originalValues[D][C]=this._originalValues[D][C]||B;
this.removeObjects(D,C);
this.addStatement(D,C,A);
this.getProperty(C)._onNewData();
this._listeners.fire("onAfterLoadingItems",[]);
};
...
delete this._spo[D];
this.setEditMode(D,false);
if(this._newItems[D]){delete this._deletedItems[D];
}else{this._deletedItems[D]=this.getItem(D);
}var B=this.getAllProperties();
...
Exhibit.Database._Impl.prototype.resetChanges=function(){this._originalValues={};
this._deletedItems={};
this._newItems={};
};
Exhibit.Database._Impl.prototype.collectChanges=function(E){E=E||Exhibit.Database.defaultIgnoredProperties;
var G=[];
for(var B in this._newItems){var I=this.getObject(B,"type");
var K=this.getObject(B,"label")||B;
var M={id:B,label:K,changeType:"added",type:I,vals:{}};
var H=this.getAllProperties();
for(var F in H){var A=H[F];
if(E.indexOf(A)!=-1){continue;
}var D=this.getObject(B,A);
if(D){M.vals[A]={newVal:D};
}}G.push(M);
}for(var B in this._originalValues){if(B in this._newItems||this.isSubmission(B)){continue;
}var I=this.getObject(B,"type");
var K=this.getObject(B,"label")||B;
var M={id:B,label:K,changeType:"modified",type:I,vals:{}};
...
M.vals[A]={oldVal:L,newVal:C};
}G.push(M);
}for(var B in this._deletedItems){var I=this._deletedItems[B].type;
var K=this._deletedItems[B].label||B;
G.push({id:B,label:K,changeType:"deleted",type:I});
}return G;
};
Exhibit.Database._Impl.prototype.acceptChanges=function(B){if(!this.isSubmission(B)){throw B+" is not a submission!";
}var C=this.getObject(B,"change");
if(C=="modification"){var A=this.getObject(B,"changedItem");
}else{if(C=="addition"){delete this._submissionRegistry[B];
this._newItems[B]=true;
}else{throw"unknown change type "+C;
}}this._listeners.fire("onAfterLoadingItems",[]);
};
...
Exhibit.JSONPImporter.googleSpreadsheetsConverter=function(Y,F,I){var E=";";
if((I)&&(typeof I!="string")){var R=Exhibit.getAttribute(I,"separator");
if(R!=null&&R.length>0){E=R;
}}var T=[];
var J={};
var O={};
var M={"text":true,"number":true,"item":true,"url":true,"boolean":true};
var Q=Y.feed.entry;
for(var W=0;
W<Q.length;
W++){var B=Q[W];
var U=B.id.$t;
var e=U.lastIndexOf("C");
var S=U.lastIndexOf("R");
Q[W]={row:parseInt(U.substring(S+1,e))-1,col:parseInt(U.substring(e+1))-1,val:B.content.$t};
}var L=0;
var K=function(){if(L<Q.length){var d=Q[L++];
var g=[d];
...
W<C.length;
W++){var A=C[W];
var P=A.val.trim().replace(/^\{/g,"").replace(/\}$/g,"").split(":");
var D=P[0].trim();
var X=P.length>1?P[1].split(","):[];
var H={single:false};
for(var a=0;
a<X.length;
a++){var f=X[a].trim();
if(f in M){H.valueType=f;
}else{if(f=="single"){H.single=true;
}}}b[A.col]=D;
J[D]=H;
}var G=null;
while((G=K())!=null){var Z={};
for(var W=0;
W<G.length;
W++){var A=G[W];
var D=b[A.col];
if(typeof D=="string"){Z[D]=A.val;
var H=J[D];
if(!H.single){var V=A.val.split(E);
for(var N=0;
N<V.length;
N++){V[N]=V[N].trim();
}Z[D]=V;
...
}}return{types:O,properties:J,items:T};
};
...
Exhibit.LensRegistry.prototype.getNormalLens=function(E,D){for(var B=0;
...
Exhibit.LensRegistry.prototype.getSubmissionLens=function(B,A){};
Exhibit.LensRegistry.prototype.getEditLens=function(C,B){var A=B.getObject(C,"type");
if(A in this._editLensTemplates){return this._editLensTemplates[A];
}else{return this._parentRegistry&&this._parentRegistry.getEditLens(C,B);
}};
Exhibit.LensRegistry.prototype.createLens=function(D,E,A){var B=new Exhibit.Lens();
if(A.getDatabase().isNewItem(D)){SimileAjax.jQuery(E).addClass("newItem");
...
Exhibit.Lens._performConstructFromLensTemplateJob=function(D){Exhibit.Lens._constructFromLensTemplateNode({"value":D.itemID},{"value":"item"},D.template.template,D.div,D.uiContext);
...
Exhibit.Lens._constructFromLensTemplateNode=function(J,M,e,D){if(typeof e=="string"){D.appendChild(document.createTextNode(e));
return ;
}var K=e.uiContext;
var P=K.getDatabase();
var H=e.children;
function Q(){if(H!=null){for(var a=0;
a<H.length;
a++){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[a],L);
}}}if(e.condition!=null){if(e.condition.test=="if-exists"){if(!e.condition.expression.testExists(J,M,"value",P)){return ;
}}else{if(e.condition.test=="if"){if(e.condition.expression.evaluate(J,M,"value",P).values.contains(true)){if(H!=null&&H.length>0){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[0],D);
}}else{if(H!=null&&H.length>1){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[1],D);
}}return ;
}else{if(e.condition.test=="select"){var C=e.condition.expression.evaluate(J,M,"value",P).values;
if(H!=null){var F=null;
for(var Y=0;
Y<H.length;
Y++){var N=H[Y];
if(N.condition!=null&&N.condition.test=="case"){if(C.contains(N.condition.value)){Exhibit.Lens._constructFromLensTemplateNode(J,M,N,D);
return ;
}}else{if(typeof N!="string"){F=N;
}}}}if(F!=null){Exhibit.Lens._constructFromLensTemplateNode(J,M,F,D);
}return ;
}}}}var L=Exhibit.Lens._constructElmtWithAttributes(e,D,P);
if(e.contentAttributes!=null){var Z=e.contentAttributes;
for(var W=0;
W<Z.length;
W++){var O=Z[W];
var C=[];
O.expression.evaluate(J,M,"value",P).values.visit(function(a){C.push(a);
});
var U=C.join(";");
...
}}}}}if(e.subcontentAttributes!=null){var G=e.subcontentAttributes;
for(var W=0;
W<G.length;
W++){var O=G[W];
var f=O.fragments;
...
}else{T+=A.evaluateSingle(J,M,"value",P).value;
}}if(O.isStyle){L.style[O.name]=T;
}else{if("class"==O.name){L.className=T;
}else{if(Exhibit.Lens._attributeValueIsSafe(O.name,T)){L.setAttribute(O.name,T);
}}}}}if(!Exhibit.params.safe){var S=e.handlers;
for(var X=0;
X<S.length;
X++){var B=S[X];
L[B.name]=B.code;
}}var E=J["value"];
if(e.control!=null){switch(e.control){case"item-link":var b=document.createElement("a");
b.innerHTML=Exhibit.l10n.itemLinkLabel;
b.href=Exhibit.Persistence.getItemLink(E);
b.target="_blank";
L.appendChild(b);
break;
case"remove-item":if(P.isNewItem(E)){if(e.tag=="a"){L.href="javascript:";
}SimileAjax.jQuery(L).click(function(){P.removeItem(E);
});
Q();
}else{D.removeChild(L);
}break;
case"start-editing":if(e.tag=="a"){L.href="javascript:";
}SimileAjax.jQuery(L).click(function(){P.setEditMode(E,true);
K.getCollection()._listeners.fire("onItemsChanged",[]);
});
Q();
break;
case"stop-editing":if((typeof E)!="string"){D.removeChild(L);
}else{if(e.tag=="a"){L.href="javascript:";
}SimileAjax.jQuery(L).click(function(){P.setEditMode(E,false);
K.getCollection()._listeners.fire("onItemsChanged",[]);
});
Q();
}break;
case"accept-changes":if(P.isSubmission(E)){if(e.tag=="a"){L.href="javascript:";
}SimileAjax.jQuery(L).click(function(){P.acceptChanges(E);
});
Q();
}else{SimileAjax.Debug.warn("accept-changes element in non-submission item");
D.removeChild(L);
}break;
}}else{if(e.content!=null){var T=e.content.evaluate(J,M,"value",P);
if(H!=null){var V={"value":T.valueType,"index":"number"};
var I=1;
var d=function(c){var a={"value":c,"index":I++};
for(var g=0;
g<H.length;
g++){Exhibit.Lens._constructFromLensTemplateNode(a,V,H[g],L);
}};
if(T.values instanceof Array){for(var W=0;
W<T.values.length;
W++){d(T.values[W]);
}}else{T.values.visit(d);
}}else{Exhibit.Lens._constructDefaultValueList(T.values,T.valueType,L,e.uiContext);
}}else{if(e.edit!=null){Q();
Exhibit.Lens._constructEditableContent(e,L,E,K);
}else{if(H!=null){for(var W=0;
W<H.length;
W++){Exhibit.Lens._constructFromLensTemplateNode(J,M,H[W],L);
}}}}}};
...
Exhibit.Lens._constructEditableContent=function(B,C,A,D){var I=D.getDatabase();
var G=B.edit;
if(A.type=="temporaryItem"){var H=A.values[G]||"";
var F=function(){A.values[G]=this.value;
};
}else{var H=I.getObject(A,G);
var F=function(){if(H!=this.value){I.editItem(A,G,this.value);
}};
}if(B.tag=="select"){for(var E in C.options){if(C.options.hasOwnProperty(E)&&C.options[E].value==H){C.selectedIndex=E;
}}}else{C.value=H;
}if(B.tag=="select"){SimileAjax.jQuery(C).blur(F);
}else{SimileAjax.jQuery(C).change(F);
}};
...
Exhibit.FacetUtilities.constructFacetFrame=function(H,E,B,A,D,G){H.className="exhibit-facet";
var F=SimileAjax.DOM.createDOMFromString(H,"<div class='exhibit-facet-header'><div class='exhibit-facet-header-filterControl' id='clearSelectionsDiv' title='"+Exhibit.FacetUtilities.l10n.clearSelectionsTooltip+"'><span id='filterCountSpan'></span><img id='checkImage' /></div>"+((D)?"<img src='"+Exhibit.urlPrefix+"images/collapse.png' class='exhibit-facet-header-collapse' id='collapseImg' />":"")+"<span class='exhibit-facet-header-title'>"+E+"</span></div><div class='exhibit-facet-body-frame' id='frameDiv'></div>",{checkImage:Exhibit.UI.createTranslucentImage("images/black-check.png")});
var C=Exhibit.ResizableDivWidget.create({},F.frameDiv,A);
F.valuesContainer=C.getContentDiv();
...
SimileAjax.WindowManager.registerEvent(F.clearSelectionsDiv,"click",B);
if(D){SimileAjax.WindowManager.registerEvent(F.collapseImg,"click",function(){Exhibit.FacetUtilities.toggleCollapse(F);
});
...
Exhibit.FacetUtilities.constructFlowingFacetFrame=function(G,D,B,A,C,F){G.className="exhibit-flowingFacet";
var E=SimileAjax.DOM.createDOMFromString(G,"<div class='exhibit-flowingFacet-header'>"+((C)?"<img src='"+Exhibit.urlPrefix+"images/collapse.png' class='exhibit-facet-header-collapse' id='collapseImg' />":"")+"<span class='exhibit-flowingFacet-header-title'>"+D+"</span></div><div id='frameDiv'><div class='exhibit-flowingFacet-body' id='valuesContainer'></div></div>");
E.setSelectionCount=function(H){};
if(C){SimileAjax.WindowManager.registerEvent(E.collapseImg,"click",function(){Exhibit.FacetUtilities.toggleCollapse(E);
});
