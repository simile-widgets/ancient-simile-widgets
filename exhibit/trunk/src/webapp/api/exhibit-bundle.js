...
Exhibit.SliderFacet._settingsSpecs={"facetLabel":{type:"text"},"scroll":{type:"boolean",defaultValue:true},"height":{type:"text"},"precision":{type:"float",defaultValue:1},"histogram":{type:"boolean",defaultValue:true},"height":{type:"int",defaultValue:false},"horizontal":{type:"boolean",defaultValue:true}};
...
}if(A){this._scaleFactor=(this._range.max-this._range.min)/this._dom.bar.offsetWidth;
}else{this._scaleFactor=(this._range.max-this._range.min)/this._dom.bar.offsetHeight;
...
I.value=parseFloat(Exhibit.Util.round(P*B._scaleFactor+B._range.min,B._prec));
B._setDisplays(I);
},0);
};
};
...
Exhibit.SliderFacet.slider.prototype.updateHistogram=function(C,B){var B=B?B:75;
