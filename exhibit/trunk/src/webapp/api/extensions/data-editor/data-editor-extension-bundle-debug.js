...
var overlay='<div class="'+Exhibit.DataEdit.EDIT_INJECT_MARKER+'" onMouseOver="Exhibit.DataEdit._rollIn_(this)" onMouseOut="Exhibit.DataEdit._rollOut_(this)" onclick="Exhibit.DataEdit.edit(\''+id+'\')" style="position:Absolute ; top:'+xy.top+"px ; left:"+xy.left+"px ; width:"+w+"px ; height:"+h+'px ; cursor:Help ; border:2px #dddddd Dotted"></div>';
$(this).append(overlay);
}});
...
Exhibit.DataEdit.edit=function(itemId){Exhibit.DataEdit._invokeEventHandlers("onBeforeEdit");
var self=this;
var filter=function(idx){return($(this).attr("ex:itemid"));
};
$("*").filter(filter).each(function(idx){var filterEdit=function(idx){return $(this).attr("_ex:role")==Exhibit.DataEdit.EDIT_ROLE_EDIT;
};
...
Exhibit.DataEdit.save=function(itemId){Exhibit.DataEdit._invokeEventHandlers("onBeforeSave");
var self=this;
...
for(var k in Exhibit.DataEdit._internalValidators){if(vl==k){Exhibit.DataEdit._internalValidators[k](val);
found=true;
}}if(!found&&(window[v])&&(typeof window[v]=="function")){window[v](val);
found=true;
}if(!found){}if(err!=null){$("#"+Exhibit.DataEdit.EDIT_MESSAGE).html(err);
f.setError(true);
return ;
...
Exhibit.DataEdit._invokeEventHandlers=function(type){for(var h in Exhibit.DataEdit._lifeCycleEventHandlers_){if(h[type]){h[type]();
}}};
...
}else{var buttonHTML='<div style="position:Fixed; right:1em;  top:1em;"><div class="'+Exhibit.DataEdit.EDIT_BUTTON+'"><a href="javascript:Exhibit.DataEdit.activate();"><span id="symbol"><span class="off">&#10006;</span></span>&nbsp;Editor</a></div></div>';
...
var tag=Exhibit.DataEdit.Editor._htmlTag("textarea",{"id":this._divId,"style":style,"class":"exhibitDataEditTextArea","rows":r,"cols":c,"onchange":onChange},$(this._jqThis).get()[0],false);
...
/* xchecklist-field.js */
Exhibit.DataEdit.Editor.CheckListField=function(jq,iid,pid,val,noLens){this._jqThis=jq;
this._itemId=iid;
this._propId=pid;
this._value=val;
this._divId="__DATAEDITOR__"+this._propId;
...
Exhibit.DataEdit.Editor.CheckListField.prototype.getHTML=function(onShow){var self=this;
onShow.push(function(){var split=function(val){return val.split(/,\s*/);
};
var extractLast=function(term){return split(term).pop();
};
...
i<this._value.length;
i++){l=l+((l.length)?",":"")+this._value[i];
}var tag=Exhibit.DataEdit.Editor._htmlTag("input",{"id":this._divId,"style":style,"class":"exhibitDataEditCheckListField","value":l},$(this._jqThis).get()[0],true);
return tag;
};
Exhibit.DataEdit.Editor.CheckListField.prototype.getValue=function(){var els=$("#"+this._divId).get();
var val=((els.length)?els[0].value:"");
var vals=val.split(/,\s*/);
var ret=[];
for(var i=0;
...
Exhibit.DataEdit.Editor.CheckListField.prototype.setValue=function(v){var els=$("#"+this._divId).get();
if(els.length){els[0].value=v.join(",");
}};
Exhibit.DataEdit.Editor.CheckListField.prototype.setError=function(b){$("#"+this._divId).css("background-color",b?Exhibit.DataEdit.Editor._ERRCOL_:Exhibit.DataEdit.Editor._BGCOL_);
};
Exhibit.DataEdit.Editor.CheckListField.domFilter=function(jqThis,f){$("select",jqThis).filter(function(idx){return $(this).attr("ex:type")=="list";
}).each(f);
};
