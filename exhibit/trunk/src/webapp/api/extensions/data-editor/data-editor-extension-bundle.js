...
if(Exhibit.DataEdit._lock_){Exhibit.DataEdit._cancel_();
Exhibit.DataEdit._lock_=false;
return ;
...
$("*").filter(B).each(function(C){var D=$(this).attr("ex:itemid");
$(this).append('<div class="'+Exhibit.DataEdit.EDIT_DIV+'"><div class="'+Exhibit.DataEdit.EDIT_BUTTON+'"><a href="javascript:Exhibit.DataEdit._edit_(\''+D+"')\">Edit</a></div></div>");
});
};
Exhibit.DataEdit._edit_=function(C){var A=this;
var B=function(D){return($(this).attr("ex:itemid"));
};
$("*").filter(B).each(function(D){$("."+Exhibit.DataEdit.EDIT_DIV,this).remove();
if($(this).attr("ex:itemid")==C){var E=new Exhibit.DataEdit.Editor(C,this);
Exhibit.DataEdit._editors_[C]=E;
E.apply();
$(this).append('<div class="'+Exhibit.DataEdit.EDIT_DIV+'"><span class="'+Exhibit.DataEdit.EDIT_BUTTON+'"><a href="javascript:Exhibit.DataEdit._save_(\''+C+'\')">Save</a></span>&nbsp;&nbsp;<span class="'+Exhibit.DataEdit.EDIT_BUTTON+'"><a href="javascript:Exhibit.DataEdit._cancel_()">Cancel</a></span>&nbsp;&nbsp;<span id="'+Exhibit.DataEdit.EDIT_MESSAGE+'"></span></div>');
}});
};
Exhibit.DataEdit._save_=function(L){var N=this;
var G=Exhibit.DataEdit.UPDATE_MODE;
var J=Exhibit.DataEdit._editors_[L];
var H=J.getFields();
for(var K in H){H[K].setError(false);
...
F++){var C=null;
var M=A[F];
if(M.charAt(0)==="#"){M=M.toLowerCase();
if(M=="#notempty"){C=Exhibit.DataEdit.validateIsNotEmpty(B);
}else{if(M=="#isurl"){C=Exhibit.DataEdit.validateIsURL(B);
}}}else{if((window[M])&&(typeof window[M]=="function")){window[M](B);
}}if(C!=null){$("#"+Exhibit.DataEdit.EDIT_MESSAGE).html(C);
I.setError(true);
return ;
}}}if((G==Exhibit.DataEdit.UPDATE_MODE)&&(B!=undefined)){database.removeObjects(L,K);
if(B instanceof Array){Exhibit.DataEdit.log("Updating(array)",K,B);
for(var E=0;
E<B.length;
E++){database.addStatement(L,K,B[E]);
}}else{Exhibit.DataEdit.log("Updating(scalar)",K,B);
database.addStatement(L,K,B);
}}else{if((G==Exhibit.DataEdit.CREATE_MODE)&&(B!=undefined)){Exhibit.DataEdit.log("Creating",K,B);
}else{if(G==Exhibit.DataEdit.DELETE_MODE){Exhibit.DataEdit.log("Deleting",K,B);
database.removeObjects(L,K);
}}}}database._listeners.fire("onAfterLoadingItems",[]);
Exhibit.DataEdit._lock_=false;
};
Exhibit.DataEdit._cancel_=function(){database._listeners.fire("onAfterLoadingItems",[]);
Exhibit.DataEdit._lock_=false;
};
Exhibit.DataEdit.deactivate=function(){var A=this;
var B=function(C){return $(this).attr("ex:itemid");
};
Exhibit.DataEdit.log("deactivate()");
$("*").filter(B).each(function(C){$(this).click(null);
});
};
...
$(document).ready(function(){var C=function(E){return $(this).attr("ex:role")=="editorButton";
};
var B=$("*").filter(C);
...
}else{var D='<div style="position:Fixed; right:1em;  top:1em;"><div class="'+Exhibit.DataEdit.EDIT_BUTTON+'"><a href="javascript:Exhibit.DataEdit.activate();">Toggle editor</a></div></div>';
...
$(document).ready(function(){var A=function(B){return $(this).attr("ex:role")=="editor";
};
...
C++){var D=Exhibit.DataEdit.Editor._COMPONENTS_[C];
A.log("Scanning for "+D);
Exhibit.DataEdit.Editor[D].domFilter(A._jqThis,function(F){var J,I,H;
try{J=A._getContent(this);
if(!J){throw"Missing target (ex:content)";
}I=(A._exists(J))?A._getValues(J):undefined;
try{H=new Exhibit.DataEdit.Editor[D](this,A._itemId,J,I);
H._validators=$(this).attr("ex:validators");
H._undefined=(I==undefined);
}catch(G){A.log(G,J,I,this);
SimileAjax.Debug.warn(G);
}A._addFieldComponent(this,J,H,E);
}catch(G){A.log(G,J,I,this);
}});
}for(var C=0;
C<E.length;
...
H=new Exhibit.DataEdit.Editor[F](this,A._itemId,K,J);
H._undefined=(J==undefined);
H._matchDisplayLens=true;
...
}else{$(D).replaceWith('<span style="color:Red;">Failed to initalise</span>');
}};
...
Exhibit.DataEdit.Editor._isTrueFalse=function(A,B){B=(B==undefined)?false:B;
if(!A){return B;
}A=A.toLowerCase();
return(A==="true"||A==="yes");
};
...
}else{if(B.parentNode!=null){return Exhibit.DataEdit.Editor._getLineHeight(B.parentNode);
}else{return 16;
}}}else{return 16;
}};
Exhibit.DataEdit.Editor._getLineHeight2=function(B){var A=document.createElement("div");
...
/* enum-field.js */
Exhibit.DataEdit.Editor.EnumField=function(F,D,B,E){this._jqThis=F;
this._itemId=D;
this._propId=B;
this._value=E;
this._divId="__DATAEDITOR__"+this._propId;
this._validators=null;
this._matchDisplayLens=false;
this._options=[];
this._allowNew=Exhibit.DataEdit.Editor._isTrueFalse($(F).attr("ex:allowNew"),true);
var A=this;
$("option",this._jqThis).each(function(G){A._options.push($(this).attr("value"));
});
...
var C=null;
if(this._matchDisplayLens){C=Exhibit.DataEdit.Editor._extractStyle(this._jqThis)+"border-width: 0px; width: "+$(this._jqThis).width()+"px; height: "+$(this._jqThis).height()+"px; ";
...
/* list-field.js */
Exhibit.DataEdit.Editor.ListField=function(F,D,B,E){this._jqThis=F;
this._itemId=D;
this._propId=B;
this._value=E;
this._divId="__DATAEDITOR__"+this._propId;
...
var E=null;
if(this._matchDisplayLens){E=Exhibit.DataEdit.Editor._extractStyle(this._jqThis)+"border-width: 0px; width: "+$(this._jqThis).width()+"px; height: "+$(this._jqThis).height()+"px; ";
}var B="";
for(var D=0;
...
/* number-field.js */
Exhibit.DataEdit.Editor.NumberField=function(D,B,A,C){this._jqThis=D;
this._itemId=B;
this._propId=A;
this._value=C;
this._divId="__DATAEDITOR__"+this._propId;
...
this._matchDisplayLens=false;
if(this._value==undefined){this._value="";
}if((typeof this._value)!="number"){if((typeof (this._value)=="string")&&(this._value.match(/^-?[0-9]*$/))){this._value=parseInt(this._value);
}else{throw"Invalid data for NumberField";
}}};
Exhibit.DataEdit.Editor.NumberField.prototype.getHTML=function(C){var B=null;
if(this._matchDisplayLens){B=Exhibit.DataEdit.Editor._extractStyle(this._jqThis)+"border-width: 0px; width: "+$(this._jqThis).width()+"px; height: "+$(this._jqThis).height()+"px; ";
}var A=Exhibit.DataEdit.Editor._htmlTag("input",{"id":this._divId,"style":B,"class":"exhibitDataEditNumberField","type":"Text","value":this._value},$(this._jqThis).get()[0],true);
return A;
};
...
/* text-field.js */
Exhibit.DataEdit.Editor.TextField=function(D,B,A,C){this._jqThis=D;
this._itemId=B;
this._propId=A;
this._value=C;
this._divId="__DATAEDITOR__"+this._propId;
this._validators=null;
this._matchDisplayLens=false;
this._largeText=Exhibit.DataEdit.Editor._isTrueFalse($(D).attr("ex:large"));
this._rows=parseInt($(D).attr("ex:rows"));
this._cols=parseInt($(D).attr("ex:cols"));
if(this._value==undefined){this._value="";
}if((typeof this._value)!="string"){throw"Invalid data for TextField";
}};
Exhibit.DataEdit.Editor.TextField.prototype.getHTML=function(G){var D=null;
if(this._matchDisplayLens){D=Exhibit.DataEdit.Editor._extractStyle(this._jqThis)+"border-width: 0px; width: "+$(this._jqThis).width()+"px; height: "+$(this._jqThis).height()+"px; ";
var C=$(this._jqThis).height();
var B=$(this._jqThis).css("line-height");
if(C&&B){B=parseInt(B.replace(/px/,""));
this._largeText=((C/B)>=2);
}}if(this._largeText){var F=this._cols?this._cols:null;
var E=this._rows?this._rows:null;
var A=Exhibit.DataEdit.Editor._htmlTag("textarea",{"id":this._divId,"style":D,"class":"exhibitDataEditTextArea","rows":E,"cols":F},$(this._jqThis).get()[0],false);
return A+this._value+"</textarea>";
}else{var A=Exhibit.DataEdit.Editor._htmlTag("input",{"id":this._divId,"style":D,"class":"exhibitDataEditTextField","type":"Text","value":this._value},$(this._jqThis).get()[0],true);
return A;
}};
