...
if(Exhibit.DataEdit._lock_){Exhibit.DataEdit._invokeEventHandlers("onBeforeActivateClose");
...
$("*").filter(B).each(function(C){var J=$(this).attr("ex:itemid");
var H=function(K){return $(this).attr("_ex:role")==Exhibit.DataEdit.EDIT_ROLE_EDIT;
};
var E=$("*",this).filter(H);
if(E.length>0){E.each(function(K){$(this).css("display","Block");
$(this).click(function(){Exhibit.DataEdit.edit(J);
});
});
}else{var I=$(this).offset();
var D=$(this).outerWidth(true);
var G=$(this).outerHeight(true);
var F='<div class="'+Exhibit.DataEdit.EDIT_INJECT_MARKER+'" onMouseOver="Exhibit.DataEdit._rollIn_(this)" onMouseOut="Exhibit.DataEdit._rollOut_(this)" onclick="Exhibit.DataEdit.edit(\''+J+'\')" style="position:Absolute ; top:'+I.top+"px ; left:"+I.left+"px ; width:"+D+"px ; height:"+G+'px ; cursor:Help ; border:2px #dddddd Dotted"></div>';
$(this).append(F);
}});
...
Exhibit.DataEdit.edit=function(C){Exhibit.DataEdit._invokeEventHandlers("onBeforeEdit");
var A=this;
var B=function(D){return($(this).attr("ex:itemid"));
};
$("*").filter(B).each(function(D){var F=function(G){return $(this).attr("_ex:role")==Exhibit.DataEdit.EDIT_ROLE_EDIT;
};
...
Exhibit.DataEdit.save=function(N){Exhibit.DataEdit._invokeEventHandlers("onBeforeSave");
var P=this;
var I=Exhibit.DataEdit.UPDATE_MODE;
var L=Exhibit.DataEdit._editors_[N];
var J=L.getFields();
for(var M in J){J[M].setError(false);
}for(var M in J){var K=J[M];
var B=K.getValue();
var F=K._validators;
if(F){var A=F.split(",");
for(var H=0;
...
var Q=false;
var C=O.toLowerCase();
for(var D in Exhibit.DataEdit._internalValidators){if(C==D){Exhibit.DataEdit._internalValidators[D](B);
Q=true;
}}if(!Q&&(window[O])&&(typeof window[O]=="function")){window[O](B);
Q=true;
}if(!Q){}if(E!=null){$("#"+Exhibit.DataEdit.EDIT_MESSAGE).html(E);
K.setError(true);
return ;
}}}if((I==Exhibit.DataEdit.UPDATE_MODE)&&(B!=undefined)){database.removeObjects(N,M);
if(B instanceof Array){Exhibit.DataEdit.log("Updating(array)",M,B);
for(var G=0;
G<B.length;
G++){database.addStatement(N,M,B[G]);
}}else{Exhibit.DataEdit.log("Updating(scalar)",M,B);
database.addStatement(N,M,B);
}}else{if((I==Exhibit.DataEdit.CREATE_MODE)&&(B!=undefined)){Exhibit.DataEdit.log("Creating",M,B);
}else{if(I==Exhibit.DataEdit.DELETE_MODE){Exhibit.DataEdit.log("Deleting",M,B);
database.removeObjects(N,M);
}}}}database._listeners.fire("onAfterLoadingItems",[]);
Exhibit.DataEdit._setEditLock(false);
Exhibit.DataEdit._invokeEventHandlers("onSave");
};
Exhibit.DataEdit.cancel=function(){Exhibit.DataEdit._invokeEventHandlers("onBeforeCancel");
database._listeners.fire("onAfterLoadingItems",[]);
Exhibit.DataEdit._setEditLock(false);
Exhibit.DataEdit._invokeEventHandlers("onCancel");
};
Exhibit.DataEdit._setEditLock=function(A){Exhibit.DataEdit._lock_=A;
$(".exhibitDataEditButton #symbol").html(Exhibit.DataEdit._lock_?'<span class="on">&#10004;</span>':'<span class="off">&#10006;</span>');
};
Exhibit.DataEdit.onChange=function(E,B){var C=Exhibit.DataEdit._editors_[E];
var A=C.getFields();
var D=A[B];
Exhibit.DataEdit._saveField(E,B,D);
};
Exhibit.DataEdit.log=function(){if(window.console&&Exhibit.DataEdit._DEBUG_){console.log.apply(this,arguments);
}};
Exhibit.DataEdit._saveField=function(L,K,J){var I=Exhibit.DataEdit.UPDATE_MODE;
var B=J.getValue();
var F=J._validators;
if(F){var A=F.split(",");
for(var H=0;
...
for(var D in Exhibit.DataEdit._internalValidators){if(C==D){Exhibit.DataEdit._internalValidators[D](B);
N=true;
}}if(!N&&(window[M])&&(typeof window[M]=="function")){window[M](B);
N=true;
}if(!N){}if(E!=null){$("#"+Exhibit.DataEdit.EDIT_MESSAGE).html(E);
J.setError(true);
return ;
}}}if((I==Exhibit.DataEdit.UPDATE_MODE)&&(B!=undefined)){database.removeObjects(L,K);
if(B instanceof Array){Exhibit.DataEdit.log("Updating(array)",K,B);
for(var G=0;
G<B.length;
G++){database.addStatement(L,K,B[G]);
}}else{Exhibit.DataEdit.log("Updating(scalar)",K,B);
database.addStatement(L,K,B);
}}else{if((I==Exhibit.DataEdit.CREATE_MODE)&&(B!=undefined)){Exhibit.DataEdit.log("Creating",K,B);
}else{if(I==Exhibit.DataEdit.DELETE_MODE){Exhibit.DataEdit.log("Deleting",K,B);
...
Exhibit.DataEdit._invokeEventHandlers=function(B){for(var A in Exhibit.DataEdit._lifeCycleEventHandlers_){if(A[B]){A[B]();
}}};
...
}else{var D='<div style="position:Fixed; right:1em;  top:1em;"><div class="'+Exhibit.DataEdit.EDIT_BUTTON+'"><a href="javascript:Exhibit.DataEdit.activate();"><span id="symbol"><span class="off">&#10006;</span></span>&nbsp;Editor</a></div></div>';
...
$(document).ready(function(){Exhibit.DataEdit._invokeEventHandlers("onBeforeInit");
...
var G=function(H){return $(this).attr("ex:role")==Exhibit.DataEdit.EDIT_ROLE_CANCEL;
};
$("*",A._jqThis).filter(G).each(function(H){$(this).click(function(){Exhibit.DataEdit.cancel();
});
...
}A._addFieldComponent(this,L,J,F);
}catch(I){A.log(I,L,K,this);
}});
}for(var C=0;
C<F.length;
...
Exhibit.DataEdit.Editor.TextField.prototype.getHTML=function(H){var E=null;
if(this._noEditorLens||this._matchDisplayLens){E=Exhibit.DataEdit.Editor._extractStyle(this._jqThis)+"border-width: 0px; width: "+$(this._jqThis).width()+"px; height: "+$(this._jqThis).height()+"px; ";
var D=$(this._jqThis).height();
var C=$(this._jqThis).css("line-height");
if(D&&C){C=parseInt(C.replace(/px/,""));
this._largeText=((D/C)>=2);
}}var B=(this._saveOnChange)?"Exhibit.DataEdit.onChange('"+this._itemId+"','"+this._propId+"')":"";
if(this._largeText){var G=this._cols?this._cols:null;
var F=this._rows?this._rows:null;
var A=Exhibit.DataEdit.Editor._htmlTag("textarea",{"id":this._divId,"style":E,"class":"exhibitDataEditTextArea","rows":F,"cols":G,"onchange":B},$(this._jqThis).get()[0],false);
return A+this._value+"</textarea>";
}else{var A=Exhibit.DataEdit.Editor._htmlTag("input",{"id":this._divId,"style":E,"class":"exhibitDataEditTextField","type":"Text","value":this._value,"onchange":B},$(this._jqThis).get()[0],true);
return A;
}};
...
/* xchecklist-field.js */
Exhibit.DataEdit.Editor.CheckListField=function(G,E,B,F,D){this._jqThis=G;
this._itemId=E;
this._propId=B;
this._value=F;
this._divId="__DATAEDITOR__"+this._propId;
...
}if(!(this._value instanceof Array)){throw"Invalid data for CheckListField";
}};
Exhibit.DataEdit.Editor.CheckListField.prototype.getHTML=function(F){var C=this;
F.push(function(){var G=function(J){return J.split(/,\s*/);
};
var I=function(J){return G(J).pop();
};
...
D<this._value.length;
D++){B=B+((B.length)?",":"")+this._value[D];
}var A=Exhibit.DataEdit.Editor._htmlTag("input",{"id":this._divId,"style":E,"class":"exhibitDataEditCheckListField","value":B},$(this._jqThis).get()[0],true);
return A;
};
Exhibit.DataEdit.Editor.CheckListField.prototype.getValue=function(){var D=$("#"+this._divId).get();
var H=((D.length)?D[0].value:"");
var E=H.split(/,\s*/);
var B=[];
for(var C=0;
...
Exhibit.DataEdit.Editor.CheckListField.prototype.setValue=function(A){var B=$("#"+this._divId).get();
if(B.length){B[0].value=A.join(",");
}};
Exhibit.DataEdit.Editor.CheckListField.prototype.setError=function(A){$("#"+this._divId).css("background-color",A?Exhibit.DataEdit.Editor._ERRCOL_:Exhibit.DataEdit.Editor._BGCOL_);
};
Exhibit.DataEdit.Editor.CheckListField.domFilter=function(A,B){$("select",A).filter(function(C){return $(this).attr("ex:type")=="list";
}).each(B);
};
