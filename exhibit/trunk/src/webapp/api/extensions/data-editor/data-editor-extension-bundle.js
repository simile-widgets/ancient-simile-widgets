...
var F='<div class="'+Exhibit.DataEdit.EDIT_INJECT_MARKER+'" onMouseOver="Exhibit.DataEdit._rollIn_(this)" onMouseOut="Exhibit.DataEdit._rollOut_(this)" style="position:Absolute ; top:'+I.top+"px ; left:"+I.left+"px ; width:"+D+"px ; height:"+G+'px ; cursor:Help ; border:2px #dddddd Dotted;"><div onclick="Exhibit.DataEdit.edit(\''+J+'\')" style="width:'+D+"px ; height:"+G+'px ; background:Black; opacity:0.0; filter:alpha(opacity=0); -ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0);"></div></div>';
$(this).append(F);
}});
...
Exhibit.DataEdit.save=function(G){Exhibit.DataEdit._invokeEventHandlers("onBeforeSave");
var C=this;
var F=Exhibit.DataEdit.UPDATE_MODE;
var D=Exhibit.DataEdit._editors_[G];
var A=D.getFields();
for(var B in A){A[B].setError(false);
}for(var B in A){var E=A[B];
if(Exhibit.DataEdit._saveField(G,B,E)){return ;
...
var A=C.getFields();
var D=A[B];
if(Exhibit.DataEdit._saveField(E,B,D)){$("."+Exhibit.DataEdit.STATUS_CLASS).each(function(F){$(this).html(err);
});
}};
...
}else{var D='<div style="position:Fixed; right:1em;  top:1em;"><div class="'+Exhibit.DataEdit.EDIT_BUTTON+'"><a href="javascript:Exhibit.DataEdit.activate();"><div><span id="symbol"><span class="off">&#10006;</span></span>&nbsp;Editor</div></a></div></div>';
...
var D=function(I){return $(this).attr("ex:role")==Exhibit.DataEdit.EDIT_ROLE_SAVE;
};
$("*",A._jqThis).filter(D).each(function(I){$(this).click(function(){Exhibit.DataEdit.save(A._itemId);
});
A._hasSaveButton=true;
});
var H=function(I){return $(this).attr("ex:role")==Exhibit.DataEdit.EDIT_ROLE_CANCEL;
};
$("*",A._jqThis).filter(H).each(function(I){$(this).click(function(){Exhibit.DataEdit.cancel();
});
A._hasCancelButton=true;
});
var G=function(I){return $(this).attr("ex:role")==Exhibit.DataEdit.EDIT_ROLE_STATUS;
};
...
if(this._noEditorLens||this._matchDisplayLens){D=Exhibit.DataEdit.Editor._extractStyle(this._jqThis)+"border-width: 0px; width: "+$(this._jqThis).width()+"px; height: "+$(this._jqThis).height()+"px; ";
var C=$(this._jqThis).height();
$(this._jqThis).prepend('<span id="__SIZE_GUESSER__">M</span>');
var F=$("#__SIZE_GUESSER__").height();
...
var A=Exhibit.DataEdit.Editor._htmlTag("textarea",{"id":this._divId,"style":D,"class":"exhibitDataEditTextAreaField","rows":E,"cols":G,"onchange":B},$(this._jqThis).get()[0],false);
return A+this._value+"</textarea>";
}else{var A=Exhibit.DataEdit.Editor._htmlTag("input",{"id":this._divId,"style":D,"class":"exhibitDataEditTextField","type":"Text","value":this._value,"onchange":B},$(this._jqThis).get()[0],true);
return A;
}};
