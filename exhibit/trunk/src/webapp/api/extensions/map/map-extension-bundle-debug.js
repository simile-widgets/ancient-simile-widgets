...
}}if(bounds&&(!settings.autoposition)&&typeof settings.zoom=="undefined"){var zoom=Math.max(0,self._map.getBoundsZoomLevel(bounds)-1);
zoom=Math.min(zoom,maxAutoZoom,settings.maxAutoZoom);
self._map.setZoom(zoom);
}if(bounds&&(!settings.autoposition)&&typeof settings.center=="undefined"){self._map.setCenter(bounds.getCenter());
}else{if(bounds&&settings.autoposition){var zoom=Math.max(0,self._map.getBoundsZoomLevel(bounds)-1);
if(settings.maxAutoZoom&&maxAutoZoom){zoom=Math.min(zoom,maxAutoZoom,settings.maxAutoZoom);
...
}switch(settings.type){case"satellite":mapOptions.mapTypeId=goole.maps.MapTypeId.SATELLITE;
break;
case"hybrid":mapOptions.mapTypeId=goole.maps.MapTypeId.HYBRID;
break;
case"terrain":mapOptions.mapTypeId=goole.maps.MapTypeId.TERRAIN;
break;
...
var marker=Exhibit.MapView._makeMarker(point,shape,color,iconSize,itemCount==1?"":itemCount.toString(),icon,self._settings);
...
if(coords.length>1){var settings=this._settings;
var borderColor=settings.borderColor!=null?settings.borderColor:color;
var polygon=new google.maps.Polygon({paths:coords,strokeColor:borderColor,strokeWeight:settings.borderWidth,strokeOpacity:settings.borderOpacity,fillColor:color,fillOpacity:opacity});
return this._addPolygonOrPolyline(itemID,polygon);
}return null;
};
Exhibit.MapView.prototype._plotPolyline=function(itemID,polylineString,color,makeLatLng){var coords=this._parsePolygonOrPolyline(polylineString,makeLatLng);
...
}var window=new google.maps.InfoWindow({content:this._createInfoWindow(items)});
...
Exhibit.MapView._markerUrlPrefix="http://service.simile-widgets.org/painter/painter?";
Exhibit.MapView._defaultMarkerShape="circle";
Exhibit.MapView._makeMarker=function(position,shape,color,iconSize,label,iconURL,settings){var extra=label.length*3;
var halfWidth=Math.ceil(settings.shapeWidth/2)+extra;
var bodyHeight=settings.shapeHeight;
var width=halfWidth*2;
var height=bodyHeight;
...
}var marker=new google.maps.Marker({icon:markerImage,shape:markerShape,shadow:shadowImage,position:position});
return marker;
};
