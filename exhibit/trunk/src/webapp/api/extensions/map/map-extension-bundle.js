...
var osm=new OpenLayers.Layer.OSM("Street","http://tah.openstreetmap.org/Tiles/tile/${z}/${x}/${y}.png",{"wrapDateLine":true});
osm.setVisibility(false);
var wms=new OpenLayers.Layer.WMS("World Map","http://labs.metacarta.com/wms/vmap0",{"layers":"basic",},{"wrapDateLine":true});
...
if(vectorLayer.length==1){vectorLayer[0].eraseFeatures(vectorLayer[0].features);
...
var hasPolygons=(accessors.getPolygon!=null);
var hasPolylines=(accessors.getPolyline!=null);
var makeLatLng=settings.latlngOrder=="latlng"?function(first,second){return new OpenLayers.LonLat(second,first).transform(self._projection,self._map.getProjectionObject());
...
var popup=new OpenLayers.Popup.FramedCloud("vectorPopup"+Math.floor(Math.random()*10000),new OpenLayers.LonLat(centroid.x,centroid.y).transform(self._projection,self._map.getProjectionObject()),null,self._createInfoWindow([itemID]).innerHTML,null,true,function(){SimileAjax.WindowManager.cancelPopups();
self._map.removePopup(this);
});
poly.popup=popup;
popup.feature=poly;
...
Exhibit.OLMapView.prototype._createInfoWindow=function(items){var selfuic=this._uiContext;
