...
var c={};
var M=(n.getColorKey!=null);
var Q=(n.getSizeKey!=null);
var D=(n.getIconKey!=null);
var p=(n.getIcon!=null);
var O=(this._getLatlng!=null);
var B=(n.getPolygon!=null);
var P=(n.getPolyline!=null);
var E=m.latlngOrder=="latlng"?function(i,e){return new google.maps.LatLng(i,e);
...
var x=[];
var t=[];
if(O){I._getLatlng(k,b,function(AA){if(AA!=null&&"lat" in AA&&"lng" in AA){e.push(AA);
}});
}if(B){n.getPolygon(k,b,function(AA){if(AA!=null){x.push(AA);
}});
}if(P){n.getPolyline(k,b,function(AA){if(AA!=null){t.push(AA);
}});
}if(e.length>0||x.length>0||t.length>0){var u=I._settings.color;
var y=null;
if(M){y=new Exhibit.Set();
n.getColorKey(k,b,function(AA){y.add(AA);
});
u=I._colorCoder.translateSet(y,Z);
}if(e.length>0){var s=null;
if(Q){s=new Exhibit.Set();
n.getSizeKey(k,b,function(AA){s.add(AA);
});
}var w=null;
if(D){w=new Exhibit.Set();
n.getIconKey(k,b,function(AA){w.add(AA);
});
}for(var r=0;
r<e.length;
r++){var q=e[r];
...
var C=function(q){var e=q.items.length;
if(!W){W=new google.maps.LatLngBounds();
}var r=I._settings.shape;
var i=I._settings.color;
if(M){i=I._colorCoder.translateSet(q.colorKeys,Z);
}var s=I._settings.iconSize;
if(Q){s=I._sizeCoder.translateSet(q.sizeKeys,V);
}var t=null;
if(e==1){if(p){n.getIcon(q.items[0],b,function(w){t=w;
});
}}if(D){t=I._iconCoder.translateSet(q.iconKeys,H);
}var v=new google.maps.LatLng(q.latlng.lat,q.latlng.lng);
if(R>q.latlng.maxAutoZoom){R=q.latlng.maxAutoZoom;
}W.extend(v);
var k=Exhibit.MapView._makeMarker(v,r,i,s,t,e==1?"":e.toString(),I._settings);
google.maps.event.addListener(k,"click",function(){I._showInfoWindow(q.items,null,k);
if(I._selectListener!=null){I._selectListener.fire({itemIDs:q.items});
}});
k.setMap(I._map);
I._overlays.push(k);
for(var u=0;
u<q.items.length;
u++){I._itemIDToMarker[q.items[u]]=k;
}};
try{for(var l in c){C(c[l]);
}}catch(o){alert(o);
}if(M){var X=this._dom.legendWidget;
var j=this._colorCoder;
var T=Z.keys.toArray().sort();
if(m.colorLegendLabel!==null){X.addLegendLabel(m.colorLegendLabel,"color");
}if(j._gradientPoints!=null){var L=this._dom.legendWidget;
...
var U=j.translate(N);
X.addEntry(U,N);
}}if(Z.others){X.addEntry(j.getOthersColor(),j.getOthersLabel());
}if(Z.mixed){X.addEntry(j.getMixedColor(),j.getMixedLabel());
}if(Z.missing){X.addEntry(j.getMissingColor(),j.getMissingLabel());
}}if(Q){var X=this._dom.legendWidget;
var F=this._sizeCoder;
var T=V.keys.toArray().sort();
if(m.sizeLegendLabel!==null){X.addLegendLabel(m.sizeLegendLabel,"size");
}if(F._gradientPoints!=null){var a=F._gradientPoints;
var h=(a[a.length-1].value-a[0].value)/5;
T=[];
for(var g=0;
g<6;
g++){T.push(Math.floor(a[0].value+h*g));
}for(var f=0;
f<T.length;
f++){var N=T[f];
var G=F.translate(N);
...
if(m.iconLegendLabel!=null){X.addLegendLabel(m.iconLegendLabel,"icon");
}for(var f=0;
f<T.length;
f++){var N=T[f];
var K=A.translate(N);
...
}}if(W&&(!m.autoposition)&&typeof m.zoom=="undefined"){var d=Math.max(0,I._map.getBoundsZoomLevel(W)-1);
d=Math.min(d,R,m.maxAutoZoom);
I._map.setZoom(d);
}if(W&&(!m.autoposition)&&typeof m.center=="undefined"){I._map.setCenter(W.getCenter());
}else{if(W&&m.autoposition){var d=Math.max(0,I._map.getBoundsZoomLevel(W)-1);
if(m.maxAutoZoom&&R){d=Math.min(d,R,m.maxAutoZoom);
}I._map.setCenter(W.getCenter());
I._map.setZoom(d);
}}};
