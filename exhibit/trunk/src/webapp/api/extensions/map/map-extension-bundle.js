...
Exhibit.OLMapView.prototype._constructMap=function(A){var C=this._settings;
if(C.mapConstructor!=null){return C.mapConstructor(A);
}else{var E=new OpenLayers.Map({"div":A,"controls":[]});
var B=new OpenLayers.Layer.WMS("Map","http://labs.metacarta.com/wms/vmap0",{layers:"basic"},{wrapDateLine:true});
var D=new OpenLayers.Layer.WMS("Satellite","http://t1.hypercube.telascience.org/cgi-bin/landsat7",{layers:"landsat7"},{wrapDateLine:true});
D.setVisibility(false);
var F=new OpenLayers.Layer.Markers("Markers");
E.addLayers([B,D,F]);
E.setCenter(new OpenLayers.LonLat(C.center[1],C.center[0]),C.zoom);
E.addControl(new OpenLayers.Control.PanPanel());
if(C.overviewControl){E.addControl(new OpenLayers.Control.OverviewMap());
}if(C.scaleControl){E.addControl(new OpenLayers.Control.ZoomPanel());
}E.addControl(new OpenLayers.Control.Navigation(C.scrollWheelZoom,true,OpenLayers.Handler.MOD_SHIFT,true));
E.addControl(new OpenLayers.Control.LayerSwitcher());
switch(C.type){case"normal":break;
case"satellite":break;
case"hybrid":break;
}E.events.register("click",null,SimileAjax.WindowManager.cancelPopups);
return E;
}};
...
Exhibit.OLMapView.prototype._clearOverlays=function(){var A=this._map.getLayersByClass("OpenLayers.Layer.Markers");
if(A.length==1){A[0].clearMarkers();
...
var B=(m.getPolygon!=null);
var P=(m.getPolyline!=null);
var E=l.latlngOrder=="latlng"?function(k,i){return new OpenLayers.LonLat(i,k);
}:function(k,i){return new OpenLayers.LonLat(k,i);
};
S.visit(function(o){var i=[];
var x=[];
var t=[];
...
}var r=I._settings.shape;
var k=I._settings.color;
if(M){k=I._colorCoder.translateSet(p.colorKeys,Z);
}var s=I._settings.iconSize;
if(Q){s=I._sizeCoder.translateSet(p.sizeKeys,V);
}var t=null;
if(i==1){if(n){m.getIcon(p.items[0],b,function(w){t=w;
});
}}if(D){t=I._iconCoder.translateSet(p.iconKeys,H);
}var t=Exhibit.OLMapView._makeIcon(r,k,s,i==1?"":i.toString(),t,I._settings);
var v=new OpenLayers.LonLat(p.latlng.lng,p.latlng.lat);
var o=new OpenLayers.Marker(v,t);
if(R>p.latlng.maxAutoZoom){R=p.latlng.maxAutoZoom;
}W.extend(v);
o.events.register("mousedown",o,function(y){var x=I._createInfoWindow(p.items);
var w=new OpenLayers.Popup.FramedCloud("markerPoup"+Math.floor(Math.random()*10000),v,new OpenLayers.Size(100,100),x.innerHTML,t,true,function(){SimileAjax.WindowManager.cancelPopups();
this.destroy();
});
I._map.addPopup(w,true);
if(I._selectListener!=null){I._selectListener.fire({itemIDs:p.items});
}OpenLayers.Event.stop(y);
});
var q=I._map.getLayersByName("Markers");
if(q.length==1){q[0].addMarker(o);
}for(var u=0;
u<p.items.length;
u++){I._itemIDToMarker[p.items[u]]=o;
}};
for(var j in c){C(c[j]);
}if(M){var X=this._dom.legendWidget;
var h=this._colorCoder;
...
Exhibit.OLMapView.prototype._plotPolygon=function(E,G,A,F){var D=this._parsePolygonOrPolyline(G,F);
if(D.length>1){var C=this._settings;
var H=C.borderColor!=null?C.borderColor:A;
var B=new GPolygon(D,H,C.borderWidth,C.borderOpacity,A,C.opacity);
return this._addPolygonOrPolyline(E,B);
...
Exhibit.OLMapView.prototype._plotPolyline=function(E,H,B,F){var D=this._parsePolygonOrPolyline(H,F);
if(D.length>1){var C=this._settings;
var G=C.borderColor!=null?C.borderColor:B;
var A=new GPolyline(D,G,C.borderWidth,C.borderOpacity);
return this._addPolygonOrPolyline(E,A);
...
Exhibit.OLMapView.prototype._addPolygonOrPolyline=function(D,C){this._map.addOverlay(C);
var A=this;
var B=function(E){A._map.openInfoWindow(E,A._createInfoWindow([D]));
if(A._selectListener!=null){A._selectListener.fire({itemIDs:[D]});
}};
...
Exhibit.OLMapView.prototype._select=function(D){var C=this;
var E=D.itemIDs[0];
var B=this._itemIDToMarker[E];
if(B){var A=new OpenLayers.Popup.FramedCloud("markerPopup"+Math.floor(Math.random()*10000),B.lonlat,new OpenLayers.Size(100,100),this._createInfoWindow([E]).innerHTML,B.icon,true,function(){SimileAjax.WindowManager.cancelPopups();
this.destroy();
});
C._map.addPopup(A,true);
}};
Exhibit.OLMapView.prototype._createInfoWindow=function(C){var B=this;
var D=this._uiContext.getDatabase();
var A=this._uiContext.getLensRegistry();
var E={};
E.getSetting=function(F){return B._uiContext.getSetting(F);
};
E.getDatabase=function(){return D;
};
E.getLensRegistry=function(){return A;
};
E.isBeingEdited=function(F){return false;
};
E.formatList=function(G,H,I,F){return B._uiContext.formatList(G,H,I,F);
};
E.format=function(H,I,F){var G=new Exhibit.Formatter._ItemFormatter(E);
G.format=function(K,J){var M=this.formatText(K);
var L=SimileAjax.DOM.createElementFromString('<a href="'+Exhibit.Persistence.getItemLink(K)+"\" class='exhibit-item' onclick='Exhibit.UI.showItemInPopup(\""+K+"\", this, exhibit._uiContext);'>"+M+"</a>");
J(L);
};
G.format(H,F);
};
return Exhibit.ViewUtilities.fillBubbleWithItems(null,C,E);
};
