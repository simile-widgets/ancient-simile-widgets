...
Exhibit.MapView._markerUrlPrefix="http://service.simile-widgets.org/painter/painter?";
Exhibit.MapView._defaultMarkerShape="circle";
Exhibit.MapView._makeMarker=function(U,B,O,S,H,A,R){var C=H.length*3;
var G=Math.ceil(R.shapeWidth/2)+C;
var L=R.shapeHeight;
var M=G*2;
var K=L;
if(S>0){M=S;
...
L=S;
R.pin=false;
}var J=["renderer=map-marker","shape="+B,"alpha="+R.shapeAlpha,"width="+M,"height="+L,"background="+O.substr(1),"label="+H];
var Q=["renderer=map-marker-shadow","shape="+B,"width="+M,"height="+L];
var P=[];
if(A!=null){J.push("icon="+A);
if(R.iconFit!="smaller"){J.push("iconFit="+R.iconFit);
}if(R.iconScale!=1){J.push("iconScale="+R.iconScale);
}if(R.iconOffsetX!=1){J.push("iconX="+R.iconOffsetX);
}if(R.iconOffsetY!=1){J.push("iconY="+R.iconOffsetY);
}}var N={};
var D={type:"poly"};
var I={};
if(R.pin){var E=R.pinHeight;
var T=Math.ceil(R.pinWidth/2);
K+=E;
P.push("pinHeight="+E);
P.push("pinWidth="+(T*2));
N.anchor=new google.maps.Point(G,K);
I.anchor=new google.maps.Point(G,K);
D.coords=[0,0,0,L,G-T,L,G,K,G+T,L,M,L,M,0];
}else{P.push("pin=false");
N.anchor=new google.maps.Point(G,Math.ceil(K/2));
I.anchor=new google.maps.Point(G,Math.ceil(K/2));
D.coords=[0,0,0,L,M,L,M,0];
}N.size=new google.maps.Size(M,K);
I.size=new google.maps.Size(M*1.5,K);
N.url=Exhibit.MapView._markerUrlPrefix+J.concat(P).join("&")+"&.png";
if(S==0){I.url=Exhibit.MapView._markerUrlPrefix+Q.concat(P).join("&")+"&.png";
}var F=new google.maps.Marker({icon:N,shape:D,shadow:I,position:U});
return F;
};
