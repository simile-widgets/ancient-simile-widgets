...
Exhibit.OLMapView.prototype._constructMap=function(A){var D=this._settings;
if(D.mapConstructor!=null){return D.mapConstructor(A);
}else{var H=new OpenLayers.Map({"div":A,"controls":[]});
var B=new OpenLayers.Layer.WMS("Map","http://labs.metacarta.com/wms/vmap0",{layers:"basic"},{wrapDateLine:true});
var G=new OpenLayers.Layer.WMS("Satellite","http://t1.hypercube.telascience.org/cgi-bin/landsat7",{layers:"landsat7"},{wrapDateLine:true});
G.setVisibility(false);
var E=new OpenLayers.Layer.Vector("Vectors",{wrapDateLine:true});
H.addLayers([B,G,E]);
H.setCenter(new OpenLayers.LonLat(D.center[1],D.center[0]),D.zoom);
H.addControl(new OpenLayers.Control.PanPanel());
if(D.overviewControl){H.addControl(new OpenLayers.Control.OverviewMap());
}if(D.scaleControl){H.addControl(new OpenLayers.Control.ZoomPanel());
}var C=this;
var F=new OpenLayers.Control.SelectFeature(E,{onSelect:function(I){C._onFeatureSelect(C,I);
},onUnselect:function(I){C._onFeatureUnselect(C,I);
}});
H.addControl(F);
F.activate();
H.addControl(new OpenLayers.Control.Navigation(D.scrollWheelZoom,true,OpenLayers.Handler.MOD_SHIFT,true));
H.addControl(new OpenLayers.Control.LayerSwitcher());
switch(D.type){case"normal":break;
case"satellite":break;
case"hybrid":break;
}H.events.register("click",null,SimileAjax.WindowManager.cancelPopups);
return H;
}};
...
var w=new OpenLayers.Geometry.Point(q.latlng.lng,q.latlng.lat);
...
var k=new OpenLayers.Popup.FramedCloud("markerPoup"+Math.floor(Math.random()*10000),new OpenLayers.LonLat(q.latlng.lng,q.latlng.lat),null,I._createInfoWindow(q.items).innerHTML,u,true,function(){SimileAjax.WindowManager.cancelPopups();
...
var H=new OpenLayers.Geometry.LineString(G);
...
if(E.length>0){var B=E[0];
B.addFeatures([F]);
}else{return null;
}var D=this;
var C=F.geometry.getCentroid();
var A=new OpenLayers.Popup.FramedCloud("vectorPopup"+Math.floor(Math.random()*10000),new OpenLayers.LonLat(C.x,C.y),null,D._createInfoWindow([G]).innerHTML,null,true,function(){SimileAjax.WindowManager.cancelPopups();
D._map.removePopup(this);
});
F.popup=A;
A.feature=F;
...
Exhibit.OLMapView.prototype._createInfoWindow=function(C){var B=this._uiContext;
var D=this._uiContext.getDatabase();
var A=this._uiContext.getLensRegistry();
var E={};
E.getSetting=function(F){return B.getSetting(F);
};
...
E.formatList=function(G,H,I,F){return B.formatList(G,H,I,F);
};
