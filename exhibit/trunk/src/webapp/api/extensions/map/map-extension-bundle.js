...
}}if(W&&(!l.autoposition)&&typeof l.zoom=="undefined"){var d=Math.max(0,I._map.getBoundsZoomLevel(W)-1);
d=Math.min(d,R,l.maxAutoZoom);
I._map.setZoom(d);
}if(W&&(!l.autoposition)&&typeof l.center=="undefined"){I._map.setCenter(W.getCenter());
}else{if(W&&l.autoposition){var d=Math.max(0,I._map.getBoundsZoomLevel(W)-1);
if(l.maxAutoZoom&&R){d=Math.min(d,R,l.maxAutoZoom);
...
}switch(C.type){case"satellite":B.mapTypeId=goole.maps.MapTypeId.SATELLITE;
break;
case"hybrid":B.mapTypeId=goole.maps.MapTypeId.HYBRID;
break;
case"terrain":B.mapTypeId=goole.maps.MapTypeId.TERRAIN;
break;
...
var B=(m.getPolygon!=null);
var P=(m.getPolyline!=null);
var E=l.latlngOrder=="latlng"?function(k,i){return new google.maps.LatLng(k,i);
}:function(k,i){return new google.maps.LatLng(i,k);
};
S.visit(function(o){var i=[];
var x=[];
var t=[];
...
var o=Exhibit.MapView._makeMarker(u,q,k,r,i==1?"":i.toString(),s,I._settings);
o.setMap(I._map);
I._overlays.push(o);
if(R>p.latlng.maxAutoZoom){R=p.latlng.maxAutoZoom;
}W.extend(u);
google.maps.event.addListener(o,"click",function(){I._showInfoWindow(p.items,null,o);
if(I._selectListener!=null){I._selectListener.fire({itemIDs:p.items});
}});
for(var t=0;
t<p.items.length;
...
if(D.length>1){var C=this._settings;
var H=C.borderColor!=null?C.borderColor:A;
var B=new google.maps.Polygon({paths:D,strokeColor:H,strokeWeight:C.borderWidth,strokeOpacity:C.borderOpacity,fillColor:A,fillOpacity:opacity});
return this._addPolygonOrPolyline(E,B);
}return null;
};
Exhibit.MapView.prototype._plotPolyline=function(E,H,B,F){var D=this._parsePolygonOrPolyline(H,F);
...
Exhibit.MapView.prototype._showInfoWindow=function(B,D,A){if(this._infoWindow){this._infoWindow.setMap(null);
}var C=new google.maps.InfoWindow({content:this._createInfoWindow(B)});
if(D){C.setPosition(D);
}C.open(this._map,A);
this._infoWindow=C;
};
Exhibit.MapView.prototype._createInfoWindow=function(A){return Exhibit.ViewUtilities.fillBubbleWithItems(null,A,this._uiContext);
...
Exhibit.MapView._markerUrlPrefix="http://service.simile-widgets.org/painter/painter?";
Exhibit.MapView._defaultMarkerShape="circle";
Exhibit.MapView._makeMarker=function(W,B,Q,U,H,A,T){var C=H.length*3;
var G=Math.ceil(T.shapeWidth/2)+C;
var N=T.shapeHeight;
var O=G*2;
var M=N;
if(U>0){O=U;
...
N=U;
T.pin=false;
}var J=["renderer=map-marker","shape="+B,"alpha="+T.shapeAlpha,"width="+O,"height="+N,"background="+Q.substr(1),"label="+H];
var S=["renderer=map-marker-shadow","shape="+B,"width="+O,"height="+N];
var R=[];
if(A!=null){J.push("icon="+A);
if(T.iconFit!="smaller"){J.push("iconFit="+T.iconFit);
}if(T.iconScale!=1){J.push("iconScale="+T.iconScale);
}if(T.iconOffsetX!=1){J.push("iconX="+T.iconOffsetX);
}if(T.iconOffsetY!=1){J.push("iconY="+T.iconOffsetY);
}}var L=new google.maps.Size(O,M);
var K=new google.maps.Size(O*1.5,M-2);
var D={type:"poly"};
var P={size:L};
var I={size:L,scaledSize:K};
if(T.pin){var E=T.pinHeight;
var V=Math.ceil(T.pinWidth/2);
M+=E;
R.push("pinHeight="+E);
R.push("pinWidth="+(V*2));
P.anchor=new google.maps.Point(G,M);
D.coords=[0,0,0,N,G-V,N,G,M,G+V,N,O,N,O,0];
}else{R.push("pin=false");
P.anchor=new google.maps.Point(G,Math.ceil(M/2));
D.coords=[0,0,0,N,O,N,O,0];
}P.url=Exhibit.MapView._markerUrlPrefix+J.concat(R).join("&")+"&.png";
if(U==0){I.url=Exhibit.MapView._markerUrlPrefix+S.concat(R).join("&")+"&.png";
}var F=new google.maps.Marker({icon:P,shape:D,shadow:I,position:W});
return F;
};
