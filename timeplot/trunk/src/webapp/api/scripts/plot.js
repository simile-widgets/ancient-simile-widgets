...
Timeplot.Plot.prototype = {
    ...
    initialize: function() {
	    if (this._showValues && this._dataSource && this._dataSource.getValue) {
	        this._valueFlag = this._timeplot.putDiv("timeplot-valueflag");
	        this._valueFlagPole = this._timeplot.putDiv("timeplot-valueflag-pole");
	        var plot = this;
    ...
    paint: function() {
        ...
        if (this._eventSource) {
            ...
            while (i.hasNext()) {
	            ...
	            if (event.isInstant) {
                    var start = this._timeGeometry.toScreen(event.getStart());
                    var end = start;
                    ctx.beginPath();
                    ctx.moveTo(start,0);
                    ...
                    var start = this._timeGeometry.toScreen(event.getStart());
                    var end = this._timeGeometry.toScreen(event.getEnd());
                    ctx.fillRect(start,0,end - start, this._canvas.height);
                }

                var div = this._timeplot.putDiv("timeplot-event-box",{
